// Module: S3plane.obj
// Source: C:\Copter\Source\Game\S3plane.cpp
// autogenerated by simcopter_tool from PDB file

// Type: /*packed*/ struct Point2d;
struct Point2d{ // packed(0x8 bytes) TI: 0x18b2
	/*+0x0*/   int32_t x;
	/*+0x4*/   int32_t y;
};

// Type: int32_t;

// Type: /*packed*/ class PlaneClass;
// VTABLE: COPTER_D 0x00593220
class PlaneClass{ // packed(0xbc bytes) TI: 0x47ea
	enum /* __unnamed */ {
		NO_X_POSITION = -1,
		STARTING_X = 3,
		ENDING_X = 125,
		NO_Y_POSITION = -1,
		STARTING_Y = 3,
		ENDING_Y = 125,
	};
	enum IntersectionTypes {
		DEAD_END = 0,
		STRAIGHT_AHEAD = 1,
		LEFT_ONLY = 2,
		RIGHT_ONLY = 4,
		LEFT_OR_STRAIGHT = 3,
		LEFT_OR_RIGHT = 6,
		RIGHT_OR_STRAIGHT = 5,
		LEFT_STRAIGHT_OR_RIGHT = 7,
	};
	enum StoppedReasons {
		NO_REASON = 0,
		TRAFFIC = 1,
		STOPPED_TRAFFIC = 2,
		PERSON_IN_WAY = 3,
		END_OF_RAIL = 4,
		INTERSECTION_FULL = 5,
	};
	enum RailTileOffsets {
		RL_CENTOFF = 0,
		RL_EDGEOFF = 131072,
	};
	enum /* __unnamed */ {
		COLLISION_SPACE = 655360,
		PLANE_SPEED = 7864320,
		PLANE_SLOWDOWN_FACTOR = 131072,
		SPEED_VARIANCE = 262144,
		MIN_ALTITUDE = 22937600,
		MAX_TIME_TO_WAIT_BEFORE_BEAMING = 3932160,
	};
	enum flagEnums {
		INITIALIZED = 0,
		PLACED = 1,
		CRASH_WHEN_READY = 2,
		CRASHING = 3,
		NUMBER_OF_FLAGS = 4,
	};
protected:
	static /*packed*/ struct Point2d lastScannedLocation;
	static int32_t sPlaneModelSet;
	static /*packed*/ class PlaneClass lsPlane;
	/*+0x4*/   char flags[4];
	/*+0x8*/   /*packed*/ struct Point3d directionVector; // 0xc bytes
	/*+0x14*/  int32_t remainingDist;
	/*+0x18*/  int32_t speed;
	/*+0x1c*/  int32_t desiredSpeed;
	/*+0x20*/  /*packed*/ struct Point2d currentLocation; // 0x8 bytes
	/*+0x28*/  /*packed*/ struct Point2d currentCell; // 0x8 bytes
	/*+0x30*/  int32_t beamDelay;
	/*+0x34*/  int32_t beamTimer;
	/*+0x38*/  int32_t altAdjustment;
	/*+0x3c*/  long missionId;
	/*+0x40*/  /*packed*/ struct Point2d crashLocation; // 0x8 bytes
	/*+0x48*/  int32_t smokeTime;
	/*+0x4c*/  long shieldCtr;
	/*+0x50*/  long missileHits;
	/*+0x54*/  long planeModel;
	/*+0x58*/  /*packed*/ struct _DYOBJ_INST dyObj; // 0x64 bytes
public:
	void PlaneClass(const /*packed*/ class PlaneClass&);
	void PlaneClass(long, long);
	void PlaneClass();
	virtual void ~PlaneClass(); // vtable+0x0
	// calltype: NearC
	static /*packed*/ class PlaneClass* CreateInstance(long, long, int32_t);
	// calltype: NearC
	static /*packed*/ class PlaneClass* CreateInstance(int32_t);
	// calltype: NearC
	static /*packed*/ class PlaneClass* GetPlanePointer(long);
	// calltype: NearC
	static void ResetAll();
	// calltype: NearC
	static void ItterateAll();
	// calltype: NearC
	static int32_t FindPlaneToCrash(long);
	// calltype: NearC
	static void HitDispatch(long, /*packed*/ struct _DYOBJ_INST*, /*packed*/ struct _DYOBJ_INST*, long, long);
	// calltype: NearC
	static int32_t MIFFLoad(void * __ptr32);
	// calltype: NearC
	static int32_t MIFFSave(void * __ptr32);
protected:
	int32_t IsPlaneInitialized();
	int32_t IsPlanePlaced();
	int32_t IsPlaneCrashing();
	int32_t IsPlaneMoving();
	long GetPlaneModel();
	const const /*packed*/ struct _DYOBJ_INST* GetPlaneDyObj();
	const /*packed*/ struct Point3d& GetDirectionVector();
	int32_t GetSpeed();
	int32_t GetDesiredSpeed();
	const /*packed*/ struct Point2d& GetCurrentLocation();
	void SetCurrentLocation(/*packed*/ struct Point2d&);
	void SetPlaneReadyToCrash(long);
	int32_t AmIaUFO();
	int32_t IsPlaneReadyToCrash();
	void SetSpeed(int32_t);
	int32_t Initialize(int32_t);
	void Reset();
	void Itterate();
	virtual void ItterateFSM(); // vtable+0x4
	virtual void AdjustSpeed(); // vtable+0x8
	virtual enum PlaneClass::StoppedReasons IsPathClear(); // vtable+0xc
	enum PlaneClass::StoppedReasons CheckDynamicObjectsAt(const /*packed*/ struct Point2d&, const /*packed*/ struct Point3d&);
	int32_t IsPlaneOutOfCameraRange();
	void SlowDown();
	void Stop();
	void MoveForward();
	int32_t HaveIReachedNextLoc();
	int32_t AmIInANewCell();
	void UnlinkFromCell(const /*packed*/ struct Point2d&);
	void LinkToCell(const /*packed*/ struct Point2d&);
	void AdjustCurrentPosition();
	void AdjustNextPosition();
	void AdjustNextAltitude();
	void UnPlacePlane();
	int32_t PlaneCollisionCheck(int32_t, /*packed*/ struct _CELL_INFO*);
	void SetCrashWhenReady();
	void PlaneSoundDriver();
	void TurnOffShields();
	void TurnOnShields();
	void HandleUFOStuff();
	// calltype: NearC
	static void InitArrays();
	// calltype: NearC
	static void SetPlaneTypesAndLocations();
	// calltype: NearC
	static void PlaceInitialPlanes();
	virtual void BeamToWithinCameraRange(); // vtable+0x10
private:
	int32_t InitializeInstance(long, long, int32_t);
public:
	/*packed*/ class PlaneClass operator=(const /*packed*/ class PlaneClass&);
};

// Type: /*packed*/ class PlaneClass (forward reference);
// VTABLE: COPTER_D 0x00593220
class PlaneClass{ // packed(0xbc bytes) TI: 0x47ea
	enum /* __unnamed */ {
		NO_X_POSITION = -1,
		STARTING_X = 3,
		ENDING_X = 125,
		NO_Y_POSITION = -1,
		STARTING_Y = 3,
		ENDING_Y = 125,
	};
	enum IntersectionTypes {
		DEAD_END = 0,
		STRAIGHT_AHEAD = 1,
		LEFT_ONLY = 2,
		RIGHT_ONLY = 4,
		LEFT_OR_STRAIGHT = 3,
		LEFT_OR_RIGHT = 6,
		RIGHT_OR_STRAIGHT = 5,
		LEFT_STRAIGHT_OR_RIGHT = 7,
	};
	enum StoppedReasons {
		NO_REASON = 0,
		TRAFFIC = 1,
		STOPPED_TRAFFIC = 2,
		PERSON_IN_WAY = 3,
		END_OF_RAIL = 4,
		INTERSECTION_FULL = 5,
	};
	enum RailTileOffsets {
		RL_CENTOFF = 0,
		RL_EDGEOFF = 131072,
	};
	enum /* __unnamed */ {
		COLLISION_SPACE = 655360,
		PLANE_SPEED = 7864320,
		PLANE_SLOWDOWN_FACTOR = 131072,
		SPEED_VARIANCE = 262144,
		MIN_ALTITUDE = 22937600,
		MAX_TIME_TO_WAIT_BEFORE_BEAMING = 3932160,
	};
	enum flagEnums {
		INITIALIZED = 0,
		PLACED = 1,
		CRASH_WHEN_READY = 2,
		CRASHING = 3,
		NUMBER_OF_FLAGS = 4,
	};
protected:
	static /*packed*/ struct Point2d lastScannedLocation;
	static int32_t sPlaneModelSet;
	static /*packed*/ class PlaneClass lsPlane;
	/*+0x4*/   char flags[4];
	/*+0x8*/   /*packed*/ struct Point3d directionVector; // 0xc bytes
	/*+0x14*/  int32_t remainingDist;
	/*+0x18*/  int32_t speed;
	/*+0x1c*/  int32_t desiredSpeed;
	/*+0x20*/  /*packed*/ struct Point2d currentLocation; // 0x8 bytes
	/*+0x28*/  /*packed*/ struct Point2d currentCell; // 0x8 bytes
	/*+0x30*/  int32_t beamDelay;
	/*+0x34*/  int32_t beamTimer;
	/*+0x38*/  int32_t altAdjustment;
	/*+0x3c*/  long missionId;
	/*+0x40*/  /*packed*/ struct Point2d crashLocation; // 0x8 bytes
	/*+0x48*/  int32_t smokeTime;
	/*+0x4c*/  long shieldCtr;
	/*+0x50*/  long missileHits;
	/*+0x54*/  long planeModel;
	/*+0x58*/  /*packed*/ struct _DYOBJ_INST dyObj; // 0x64 bytes
public:
	void PlaneClass(const /*packed*/ class PlaneClass&);
	void PlaneClass(long, long);
	void PlaneClass();
	virtual void ~PlaneClass(); // vtable+0x0
	// calltype: NearC
	static /*packed*/ class PlaneClass* CreateInstance(long, long, int32_t);
	// calltype: NearC
	static /*packed*/ class PlaneClass* CreateInstance(int32_t);
	// calltype: NearC
	static /*packed*/ class PlaneClass* GetPlanePointer(long);
	// calltype: NearC
	static void ResetAll();
	// calltype: NearC
	static void ItterateAll();
	// calltype: NearC
	static int32_t FindPlaneToCrash(long);
	// calltype: NearC
	static void HitDispatch(long, /*packed*/ struct _DYOBJ_INST*, /*packed*/ struct _DYOBJ_INST*, long, long);
	// calltype: NearC
	static int32_t MIFFLoad(void * __ptr32);
	// calltype: NearC
	static int32_t MIFFSave(void * __ptr32);
protected:
	int32_t IsPlaneInitialized();
	int32_t IsPlanePlaced();
	int32_t IsPlaneCrashing();
	int32_t IsPlaneMoving();
	long GetPlaneModel();
	const const /*packed*/ struct _DYOBJ_INST* GetPlaneDyObj();
	const /*packed*/ struct Point3d& GetDirectionVector();
	int32_t GetSpeed();
	int32_t GetDesiredSpeed();
	const /*packed*/ struct Point2d& GetCurrentLocation();
	void SetCurrentLocation(/*packed*/ struct Point2d&);
	void SetPlaneReadyToCrash(long);
	int32_t AmIaUFO();
	int32_t IsPlaneReadyToCrash();
	void SetSpeed(int32_t);
	int32_t Initialize(int32_t);
	void Reset();
	void Itterate();
	virtual void ItterateFSM(); // vtable+0x4
	virtual void AdjustSpeed(); // vtable+0x8
	virtual enum PlaneClass::StoppedReasons IsPathClear(); // vtable+0xc
	enum PlaneClass::StoppedReasons CheckDynamicObjectsAt(const /*packed*/ struct Point2d&, const /*packed*/ struct Point3d&);
	int32_t IsPlaneOutOfCameraRange();
	void SlowDown();
	void Stop();
	void MoveForward();
	int32_t HaveIReachedNextLoc();
	int32_t AmIInANewCell();
	void UnlinkFromCell(const /*packed*/ struct Point2d&);
	void LinkToCell(const /*packed*/ struct Point2d&);
	void AdjustCurrentPosition();
	void AdjustNextPosition();
	void AdjustNextAltitude();
	void UnPlacePlane();
	int32_t PlaneCollisionCheck(int32_t, /*packed*/ struct _CELL_INFO*);
	void SetCrashWhenReady();
	void PlaneSoundDriver();
	void TurnOffShields();
	void TurnOnShields();
	void HandleUFOStuff();
	// calltype: NearC
	static void InitArrays();
	// calltype: NearC
	static void SetPlaneTypesAndLocations();
	// calltype: NearC
	static void PlaceInitialPlanes();
	virtual void BeamToWithinCameraRange(); // vtable+0x10
private:
	int32_t InitializeInstance(long, long, int32_t);
public:
	/*packed*/ class PlaneClass operator=(const /*packed*/ class PlaneClass&);
};

// Type: void ();

// Type: void;

// Type: long;

// Type: /*packed*/ struct VRview;
struct VRview{ // packed(0x58 bytes) TI: 0x2ecc
	/*+0x0*/   /*packed*/ struct Point3d loc; // 0xc bytes
	/*+0xc*/   /*packed*/ struct Point3d scale; // 0xc bytes
	/*+0x18*/  int32_t matrix[4][4]; // 0x40 bytes
};

// Type: /*packed*/ struct Point3d;
struct Point3d{ // packed(0xc bytes) TI: 0x18b0
	/*+0x0*/   int32_t x;
	/*+0x4*/   int32_t y;
	/*+0x8*/   int32_t z;
};

// Type: enum PlaneClass::StoppedReasons;
enum StoppedReasons {
	NO_REASON = 0,
	TRAFFIC = 1,
	STOPPED_TRAFFIC = 2,
	PERSON_IN_WAY = 3,
	END_OF_RAIL = 4,
	INTERSECTION_FULL = 5,
};

// Type: /*packed*/ struct Point2d (forward reference);
struct Point2d{ // packed(0x8 bytes) TI: 0x18b2
	/*+0x0*/   int32_t x;
	/*+0x4*/   int32_t y;
};

// Type: /*packed*/ struct Point3d (forward reference);
struct Point3d{ // packed(0xc bytes) TI: 0x18b0
	/*+0x0*/   int32_t x;
	/*+0x4*/   int32_t y;
	/*+0x8*/   int32_t z;
};

// Type: /*packed*/ struct _CELL_INFO (forward reference);
struct _CELL_INFO{ // packed(0x18 bytes) TI: 0x1b03
	/*+0x0*/   short flags; // 0x2 bytes
	/*+0x2*/   short x; // 0x2 bytes
	/*+0x4*/   short y; // 0x2 bytes
	/*+0x6*/   short z; // 0x2 bytes
	/*+0x8*/   short size; // 0x2 bytes
	/*+0xa*/   short ctr; // 0x2 bytes
	/*+0xc*/   /*packed*/ struct _STOBJ_INST *stptr;
	/*+0x10*/  /*packed*/ struct _DYOBJ_INST *dyptr;
	/*+0x14*/  /*packed*/ struct _DYOBJ_INST *vwptr;
};

// Type: /*packed*/ struct _MISSION_PARMS;
struct _MISSION_PARMS{ // packed(0x18 bytes) TI: 0x307d
	/*+0x0*/   long op;
	/*+0x4*/   long id;
	/*+0x8*/   /*packed*/ struct Point2d maploc; // 0x8 bytes
	/*+0x10*/  long i2num;
	/*+0x14*/  long flags;
};

// Type: /*packed*/ struct _STOBJ_INST (forward reference);
struct _STOBJ_INST{ // packed(0x10 bytes) TI: 0x193f
	/*+0x0*/   /*packed*/ struct _STOBJ_INST *next;
	/*+0x4*/   void * __ptr32 mesh;
	/*+0x8*/   long user1;
	/*+0xc*/   long user2;
};

// Type: /*packed*/ struct VRObjInfo;
struct VRObjInfo{ // packed(0x24 bytes) TI: 0x2ea8
	/*+0x0*/   int32_t Faces;
	/*+0x4*/   int32_t Verts;
	/*+0x8*/   int32_t Attribute;
	/*+0xc*/   int32_t Radius;
	/*+0x10*/  /*packed*/ struct Point3d *ObjCenter;
	/*+0x14*/  /*packed*/ struct Point3d *VertsPtr;
	/*+0x18*/  /*packed*/ struct Xform3d *VertsXfm;
	/*+0x1c*/  /*packed*/ struct Point3d *OrgVerts;
	/*+0x20*/  int32_t *Matrix[4][4];
};

// Type: /*packed*/ struct _DYOBJ_INST (forward reference);
struct _DYOBJ_INST{ // packed(0x64 bytes) TI: 0x1deb
	/*+0x0*/   /*packed*/ struct _DYOBJ_INST *next;
	/*+0x4*/   /*packed*/ struct _DYOBJ_INST *vnext;
	/*+0x8*/   void * __ptr32 mesh;
	/*+0xc*/   short flags; // 0x2 bytes
	/*+0xe*/   short user1; // 0x2 bytes
	/*+0x10*/  long radius;
	/*+0x14*/  long height;
	/*+0x18*/  /*packed*/ struct Point3d loc; // 0xc bytes
	/*+0x24*/  int32_t matrix[4][4]; // 0x40 bytes
};

// Type: /*packed*/ struct VRFaceInfo;
struct VRFaceInfo{ // packed(0x20 bytes) TI: 0x30c0
	/*+0x0*/   int32_t Face;
	/*+0x4*/   int32_t Verts;
	/*+0x8*/   int32_t Attribute;
	/*+0xc*/   int32_t Plotter;
	/*+0x10*/  /*packed*/ struct VRBmpHdr *Bitmap;
	/*+0x14*/  int32_t * VertList;
	/*+0x18*/  /*packed*/ struct MapVert *MapVList;
	/*+0x1c*/  /*packed*/ struct MapVert *BarryPtr;
};

// Type: void * __ptr32;

// Type: uint32_t;



// Contribution: 1:0012a0c0-0012cd5c Module: 163, 16 byte alignment, code, execute, read, 
// STATIC INITIALIZER:
// SYNTHETIC: COPTER_D 0x0052b0c0
static void $E8() {

	$E4();
	$E7();
	__asm        jmp    __RETURN;
__RETURN:
}

// STATIC INITIALIZER:
// SYNTHETIC: COPTER_D 0x0052b0da
static void $E4() {

	PlaneClass::lsPlane<vftable> = 0x593220;
	__asm        jmp    _T15;
_T15:
	return;
}

// STATIC INITIALIZER:
// SYNTHETIC: COPTER_D 0x0052b0f9
static void $E7() {

	atexit(0x52b116);
	__asm        jmp    __RETURN;
__RETURN:
}

// STATIC INITIALIZER:
// SYNTHETIC: COPTER_D 0x0052b116
static void $E5() {

	__asm        xor    eax, eax;
	__asm        mov    al, ds:[0x62B7B0];
	__asm        test   al, 1;
	__asm        jne    _T2d;

	__asm        xor    eax, eax;
	__asm        mov    al, ds:[0x62B7B0];
	__asm        or     al, 1;
	__asm        mov    ds:[0x62B7B0], al;
	__asm        mov    ecx, 0x62B6E8;
	__asm        call   PlaneClass::~PlaneClass;
_T2d:
	return;
}

// FUNCTION: COPTER_D 0x0052b14d
void PlaneClass::PlaneClass(long mapx, long mapy) {
	/*bp-0x4*/   int32_t currentFlag;


	this-><vftable> = 0x593220;
// LINE 177:
_FOR_21:
	for (currentFlag = 0x0; (currentFlag < 0x4); currentFlag++) {
		// LINE 179:
			__asm        mov    eax, currentFlag;
			__asm        mov    ecx, this;
			__asm        mov    byte ptr [eax+ecx+4], 0;
	}
// LINE 182:
_T3e:
	this->dyObj.mesh = 0x0;
// LINE 183:
	this->beamTimer = 0x0;
// LINE 184:
	this->speed = 0x0;
// LINE 185:
	this->desiredSpeed = 0x0;
// LINE 186:
	this->currentLocation.x = mapx;
// LINE 187:
	this->currentLocation.y = mapy;
// LINE 188:
	this->currentCell.x = mapx;
// LINE 189:
	this->currentCell.y = mapy;
// LINE 196:
	__asm        cmp    PlaneClass::sPlaneModelSet, 0;
	__asm        jne    _Tb0;
// LINE 198:
	this->planeModel = 0x12e;
// LINE 199:
	PlaneClass::sPlaneModelSet = 0x1;
// LINE 201:
	__asm        jmp    _Tba;
// LINE 203:
_Tb0:
	this->planeModel = 0x17c;
// LINE 205:
_Tba:
	return;

	__asm        mov    eax, this;
}

// FUNCTION: COPTER_D 0x0052b216
void PlaneClass::~PlaneClass() {

	this-><vftable> = 0x593220;
// LINE 229:
	__asm        jmp    _T1a;
_T1a:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+4];
	__asm        test   eax, eax;
	__asm        je     _T4c;
// LINE 231:
	__asm        jmp    _T2e;
_T2e:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        je     _T4c;
// LINE 233:
	this->PlaneClass::UnlinkFromCell((this + 0x20));
// LINE 245:
_T4c:
	this->flags[0] = 0x0;
// LINE 246:
	return;
}

// FUNCTION: COPTER_D 0x0052b273
/*packed*/ class PlaneClass* PlaneClass::GetPlanePointer(long index) {
// LINE 266:
	__asm        cmp    index, 2;
	__asm        jl     _T17;
// LINE 267:
	return 0x0;
// LINE 269:
_T17:
	__asm        mov    eax, index;
	__asm        mov    eax, planes[0][eax*4];
	__asm        jmp    __RETURN;
// LINE 270:
__RETURN:
}

// FUNCTION: COPTER_D 0x0052b29e
/*packed*/ class PlaneClass* PlaneClass::CreateInstance(int32_t instanceID) {
// LINE 290:
	return PlaneClass::CreateInstance(-0x1, -0x1, instanceID);
// LINE 328:
}

// FUNCTION: COPTER_D 0x0052b2be
/*packed*/ class PlaneClass* PlaneClass::CreateInstance(long mapx, long mapy, int32_t instanceID) {
	/*bp-0x4*/   /*packed*/ class PlaneClass *newplane;

// LINE 353:
	__asm        push   0xBC;
	__asm        call   operator new;
	__asm        add    esp, 4;
	__asm        mov    [ebp-8], eax;
	__asm        cmp    dword ptr [ebp-8], 0;
	__asm        je     _T3b;

	__asm        mov    eax, mapy;
	__asm        push   eax;
	__asm        mov    eax, mapx;
	__asm        push   eax;
	__asm        mov    ecx, [ebp-8];
	__asm        call   PlaneClass::PlaneClass;
	__asm        mov    newplane, eax;
	__asm        jmp    _T42;
_T3b:
	newplane = 0x0;
// LINE 357:
_T42:
	__asm        cmp    newplane, 0;
	__asm        je     _T94;
// LINE 361:
	__asm        mov    eax, instanceID;
	__asm        push   eax;
	__asm        mov    ecx, newplane;
	__asm        call   PlaneClass::Initialize;
	__asm        test   eax, eax;
	__asm        je     _T6d;
// LINE 363:
	return newplane;
// LINE 365:
	__asm        jmp    _T94;
// LINE 369:
_T6d:
	__asm        mov    eax, newplane;
	__asm        mov    [ebp-0x10], eax;
	__asm        mov    eax, [ebp-0x10];
	__asm        mov    [ebp-0xC], eax;
	__asm        cmp    dword ptr [ebp-0xC], 0;
	__asm        je     _T94;

	__asm        push   1;
	__asm        mov    eax, [ebp-0xC];
	__asm        mov    eax, [eax];
	__asm        mov    ecx, [ebp-0xC];
	__asm        call   dword ptr [eax];
	__asm        jmp    _T94;
// LINE 376:
_T94:
	return 0x0;
// LINE 377:
}

// FUNCTION: COPTER_D 0x0052b35e
int32_t PlaneClass::Initialize(int32_t instanceID) {
// LINE 453:
	return this->PlaneClass::InitializeInstance(this->currentLocation.x, this->currentLocation.y, instanceID);
// LINE 454:
}

// FUNCTION: COPTER_D 0x0052b390
void PlaneClass::ResetAll() {
	/*bp-0x4*/   int32_t currentPlaneIndex;

// LINE 498:
_FOR_15:
	for (currentPlaneIndex = 0x0; (currentPlaneIndex < 0x2); currentPlaneIndex++) {
		// LINE 500:
			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    ecx, planes[0][eax*4];
			__asm        call   PlaneClass::Reset;
	}
// LINE 503:
_T36:
	return;
}

// FUNCTION: COPTER_D 0x0052b3d0
void PlaneClass::ItterateAll() {
	/*bp-0x4*/   int32_t currentPlaneIndex;

// LINE 548:
_FOR_15:
	for (currentPlaneIndex = 0x0; (currentPlaneIndex < 0x2); currentPlaneIndex++) {
		// LINE 550:
			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    ecx, planes[0][eax*4];
			__asm        call   PlaneClass::Itterate;
	}
// LINE 553:
_T36:
	return;
}

// FUNCTION: COPTER_D 0x0052b410
int32_t PlaneClass::FindPlaneToCrash(long mission_id) {
	/*bp-0x4*/   int32_t currentPlaneIndex;

// LINE 580:
_FOR_15:
	for (currentPlaneIndex = 0x0; (currentPlaneIndex < 0x2); currentPlaneIndex++) {
		// LINE 584:
			__asm        jmp    _T27;
		_T27:
			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    eax, planes[0][eax*4];
			__asm        movsx  eax, byte ptr [eax+6];
			__asm        test   eax, eax;
			__asm        jne    _Tb4;

			__asm        jmp    _T42;
		_T42:
			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    eax, planes[0][eax*4];
			__asm        movsx  eax, byte ptr [eax+7];
			__asm        test   eax, eax;
			__asm        jne    _Tb4;

			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    eax, planes[0][eax*4];
			__asm        cmp    dword ptr [eax+0x54], 0x12E;
			__asm        jne    _T74;

			__asm        jmp    _T88;
		_T74:
			__asm        jmp    _Tb4;

			__asm        jmp    _T88;

			__asm        cmp    dword ptr [ebp-8], 0;
			__asm        jne    _Tb4;
		// LINE 585:
		_T88:
			__asm        mov    eax, currentPlaneIndex;
			__asm        mov    eax, planes[0][eax*4];
			__asm        mov    [ebp-0xC], eax;
		// LINE 586:
			__asm        mov    eax, [ebp-0xC];
			__asm        mov    byte ptr [eax+6], 1;
			__asm        mov    eax, mission_id;
			__asm        mov    ecx, [ebp-0xC];
			__asm        mov    [ecx+0x3C], eax;
			__asm        jmp    _Taa;
		// LINE 587:
		_Taa:
			return 0x1;
		// LINE 589:
		_Tb4:
	}
// LINE 591:
_Tb9:
	return 0x0;
// LINE 592:
}

// FUNCTION: COPTER_D 0x0052b4d5
void PlaneClass::Itterate() {
// LINE 614:
	__asm        jmp    _T11;
_T11:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+4];
	__asm        test   eax, eax;
	__asm        je     _T136;
// LINE 617:
	__asm        jmp    _T25;
_T25:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        jne    _Tb8;
// LINE 621:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T4e;

	__asm        jmp    _T7c;

	__asm        jmp    _T5d;
_T4e:
	__asm        jmp    _T5d;

	__asm        cmp    dword ptr [ebp-0x5C], 0;
	__asm        je     _T7c;
// LINE 623:
_T5d:
	__asm        cmp    G_apache_placed, 0;
	__asm        je     _T77;

	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x50], 0xA;
	__asm        jl     _T7c;
// LINE 624:
_T77:
	return;
// LINE 627:
_T7c:
	this->beamTimer += LoopTime;
// LINE 628:
	__asm        jmp    _T8c;
_T8c:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+7];
	__asm        test   eax, eax;
	__asm        jne    _Tb8;

	__asm        mov    eax, this;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x34];
	__asm        cmp    [eax+0x30], ecx;
	__asm        jge    _Tb8;
// LINE 631:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax];
	__asm        mov    ecx, this;
	__asm        call   dword ptr [eax+0x10];
// LINE 635:
_Tb8:
	__asm        jmp    _Tbd;
_Tbd:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        je     _T129;
// LINE 641:
// Block start:
	/*bp-0x58*/  /*packed*/ struct VRview position; // 0x58 bytes
	__asm        mov    eax, this;
	__asm        mov    eax, [eax];
	__asm        mov    ecx, this;
	__asm        call   dword ptr [eax+4];
// LINE 645:
	__asm        jmp    _Tdc;
_Tdc:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        je     _T129;
// LINE 648:
	__asm        mov    eax, this;
	__asm        add    eax, 0x70;
	__asm        lea    ecx, position.loc.x;
	__asm        mov    edx, [eax];
	__asm        mov    [ecx], edx;
	__asm        mov    edx, [eax+4];
	__asm        mov    [ecx+4], edx;
	__asm        mov    eax, [eax+8];
	__asm        mov    [ecx+8], eax;
// LINE 652:
	__asm        mov    esi, this;
	__asm        lea    edi, position.matrix[0][0];
	__asm        add    esi, 0x7C;
	__asm        mov    ecx, 0x10;
	__asm        rep movsd;
// LINE 656:
	_VRPositionObj(this->dyObj.mesh, position.loc.x, 0x3);
// LINE 661:
// Block end:
_T129:
	this->PlaneClass::PlaneSoundDriver();
// LINE 663:
	__asm        jmp    _T16b;
// LINE 667:
_T136:
	__asm        jmp    _T13b;
_T13b:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+4];
	__asm        test   eax, eax;
	__asm        jne    _T166;

	_assert(0x5b76c4, 0x5b76a0, 0x29b);
	__asm        jmp    _T16b;
_T166:
	__asm        jmp    _T16b;
// LINE 669:
_T16b:
	return;
}

// FUNCTION: COPTER_D 0x0052b64a
void PlaneClass::Reset() {
// LINE 689:
	this->PlaneClass::UnPlacePlane();
// LINE 691:
	this->flags[2] = 0x0;
// LINE 692:
	this->flags[3] = 0x0;
// LINE 694:
	this->speed = 0x0;
// LINE 695:
	this->missionId = 0xffffffff;
// LINE 696:
	this->crashLocation.x = 0xffffffff;
// LINE 697:
	this->crashLocation.y = 0xffffffff;
// LINE 698:
	this->currentLocation.x = 0xffffffff;
// LINE 699:
	this->currentLocation.y = 0xffffffff;
// LINE 700:
	this->currentCell.x = 0xffffffff;
// LINE 701:
	this->currentCell.y = 0xffffffff;
// LINE 702:
	this->missileHits = 0x0;
// LINE 704:
	return;
}

// FUNCTION: COPTER_D 0x0052b6d0
void PlaneClass::PlaneSoundDriver() {
	/*bp-0x4*/   int32_t dist;
	/*bp-0x10*/  /*packed*/ struct Point3d vec; // 0xc bytes
	/*bp-0x14*/  int32_t vol_adj;

// LINE 750:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T21;

	__asm        jmp    _T35;
_T21:
	__asm        jmp    _T1d3;

	__asm        jmp    _T35;

	__asm        cmp    dword ptr [ebp-0x18], 0;
	__asm        jne    _T1d3;
// LINE 753:
_T35:
	__asm        jmp    _T3a;
_T3a:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        jne    _T88;
// LINE 755:
	__asm        push   0x1C;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T66;
// LINE 757:
	S3DSStopPlay(0x1c);
// LINE 759:
_T66:
	__asm        push   0x1B;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T83;
// LINE 761:
	S3DSStopPlay(0x1b);
// LINE 763:
_T83:
	return;
// LINE 767:
_T88:
	vec.x = (ViewState.world_pos.x - this->dyObj.loc.x);
// LINE 768:
	vec.y = (ViewState.world_pos.y - this->dyObj.loc.y);
// LINE 769:
	vec.z = (ViewState.world_pos.z - this->dyObj.loc.z);
// LINE 770:
	dist = MTNormalize(vec.x);
// LINE 772:
	__asm        cmp    dist, 0x7800000;
	__asm        jge    _T194;
// LINE 774:
	__asm        push   0x1C;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        test   eax, eax;
	__asm        jne    _Tf3;
// LINE 776:
	S3DSPlay(0x1c, (this + 0x70), 0x1);
// LINE 780:
_Tf3:
	__asm        push   0xF0600000;
	__asm        push   0x7800000;
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        call   _FixedDiv;
	__asm        add    esp, 8;
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        sar    eax, 0x10;
	__asm        mov    vol_adj, eax;
// LINE 783:
	S3SoundAdjVol(0x1c, vol_adj);
// LINE 785:
	__asm        jmp    _T12b;
_T12b:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+7];
	__asm        test   eax, eax;
	__asm        je     _T172;
// LINE 787:
	__asm        push   0x1B;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        test   eax, eax;
	__asm        jne    _T15f;
// LINE 789:
	S3DSPlay(0x1b, (this + 0x70), 0x1);
// LINE 791:
_T15f:
	S3SoundAdjVol(0x1b, vol_adj);
// LINE 793:
	__asm        jmp    _T18f;
// LINE 795:
_T172:
	__asm        push   0x1B;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T18f;
// LINE 797:
	S3DSStopPlay(0x1b);
// LINE 801:
_T18f:
	__asm        jmp    _T1ce;
// LINE 803:
_T194:
	__asm        push   0x1C;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T1b1;
// LINE 805:
	S3DSStopPlay(0x1c);
// LINE 807:
_T1b1:
	__asm        push   0x1B;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T1ce;
// LINE 809:
	S3DSStopPlay(0x1b);
// LINE 813:
_T1ce:
	__asm        jmp    _T2c9;
// LINE 816:
_T1d3:
	__asm        jmp    _T1d8;
_T1d8:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        jne    _T209;
// LINE 818:
	__asm        push   0x29;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T204;
// LINE 820:
	S3DSStopPlay(0x29);
// LINE 822:
_T204:
	return;
// LINE 826:
_T209:
	vec.x = (ViewState.world_pos.x - this->dyObj.loc.x);
// LINE 827:
	vec.y = (ViewState.world_pos.y - this->dyObj.loc.y);
// LINE 828:
	vec.z = (ViewState.world_pos.z - this->dyObj.loc.z);
// LINE 829:
	dist = MTNormalize(vec.x);
// LINE 831:
	__asm        cmp    dist, 0x7800000;
	__asm        jge    _T2ac;
// LINE 833:
	__asm        push   0x29;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        test   eax, eax;
	__asm        jne    _T274;
// LINE 835:
	S3DSPlay(0x29, (this + 0x70), 0x1);
// LINE 839:
_T274:
	__asm        push   0xF0600000;
	__asm        push   0x7800000;
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        call   _FixedDiv;
	__asm        add    esp, 8;
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        sar    eax, 0x10;
	__asm        mov    vol_adj, eax;
// LINE 842:
	S3SoundAdjVol(0x29, vol_adj);
// LINE 844:
	__asm        jmp    _T2c9;
// LINE 846:
_T2ac:
	__asm        push   0x29;
	__asm        call   S3SoundIsPlaying;
	__asm        add    esp, 4;
	__asm        cmp    eax, 1;
	__asm        jne    _T2c9;
// LINE 848:
	S3DSStopPlay(0x29);
// LINE 852:
_T2c9:
	return;
}

// FUNCTION: COPTER_D 0x0052b9a3
void PlaneClass::ItterateFSM() {
// LINE 872:
	__asm        mov    ecx, this;
	__asm        call   PlaneClass::IsPlaneOutOfCameraRange;
	__asm        test   eax, eax;
	__asm        je     _T3d;

	__asm        jmp    _T21;
_T21:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+7];
	__asm        test   eax, eax;
	__asm        jne    _T3d;
// LINE 875:
	this->PlaneClass::UnPlacePlane();
// LINE 876:
	return;
// LINE 880:
_T3d:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T57;

	__asm        jmp    _T6e;

	__asm        jmp    _T66;
_T57:
	__asm        jmp    _T66;

	__asm        cmp    dword ptr [ebp-4], 0;
	__asm        je     _T6e;
// LINE 882:
_T66:
	this->PlaneClass::HandleUFOStuff();
// LINE 885:
_T6e:
	__asm        jmp    _T73;
_T73:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+6];
	__asm        test   eax, eax;
	__asm        je     _T8a;
// LINE 887:
	this->PlaneClass::SetCrashWhenReady();
// LINE 892:
_T8a:
	__asm        jmp    _T8f;
_T8f:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+7];
	__asm        test   eax, eax;
	__asm        je     _Tab;
// LINE 894:
	this->PlaneClass::MoveForward();
// LINE 895:
	return;
// LINE 898:
_Tab:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax];
	__asm        mov    ecx, this;
	__asm        call   dword ptr [eax+0xC];
	__asm        mov    [ebp-0xC], eax;
	__asm        jmp    _T139;
// LINE 904:
_Tbe:
	this->PlaneClass::MoveForward();
// LINE 907:
	__asm        mov    ecx, this;
	__asm        call   PlaneClass::HaveIReachedNextLoc;
	__asm        test   eax, eax;
	__asm        je     _T124;
// LINE 910:
	this->PlaneClass::UnlinkFromCell((this + 0x20));
// LINE 911:
	this->currentLocation.x = ((this->dyObj.loc.x + 0x20000000) >> 0x16);
// LINE 912:
	this->currentLocation.y = ((0x20000000 - this->dyObj.loc.z) >> 0x16);
// LINE 913:
	this->PlaneClass::LinkToCell((this + 0x20));
// LINE 914:
	this->PlaneClass::AdjustNextAltitude();
// LINE 920:
_T124:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax];
	__asm        mov    ecx, this;
	__asm        call   dword ptr [eax+8];
// LINE 922:
	__asm        jmp    _T148;
// LINE 931:
_T134:
	__asm        jmp    _T148;
_T139:
	__asm        cmp    dword ptr [ebp-0xC], 0;
	__asm        je     _Tbe;

	__asm        jmp    _T134;
// LINE 932:
_T148:
	return;
}

// FUNCTION: COPTER_D 0x0052baf5
void PlaneClass::AdjustSpeed() {
	// StaticLocal: 0x005b769c
	static int32_t speedAdjustor = 1;
	;

// LINE 954:
	this->speed = this->desiredSpeed;
// LINE 956:
	__asm        cmp    speedAdjustor, 0;
	__asm        jne    _T2f;
// LINE 958:
	this->speed = 0x1;
// LINE 960:
_T2f:
	return;
}

// FUNCTION: COPTER_D 0x0052bb2e
enum PlaneClass::StoppedReasons PlaneClass::IsPathClear() {
// LINE 1000:
	return 0x0;
// LINE 1001:
}

// FUNCTION: COPTER_D 0x0052bb46
enum PlaneClass::StoppedReasons PlaneClass::CheckDynamicObjectsAt(const /*packed*/ struct Point2d& __formal, const /*packed*/ struct Point3d& collisionPoint) {
// LINE 1160:
	return 0x0;
// LINE 1161:
}

// FUNCTION: COPTER_D 0x0052bb60
void PlaneClass::SetCrashWhenReady() {
	/*bp-0x4*/   int32_t dist;
	/*bp-0x8*/   int32_t x;
	/*bp-0x20*/  /*packed*/ struct _MISSION_PARMS mp; // 0x18 bytes
	/*bp-0x24*/  int32_t z;
	/*bp-0x28*/  /*packed*/ struct _CELL_INFO *cptr;
	/*bp-0x2c*/  int32_t deltaX;
	/*bp-0x30*/  int32_t deltaY;

// LINE 1186:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T26;

	__asm        jmp    _T5d;

	__asm        jmp    _T35;
_T26:
	__asm        jmp    _T35;

	__asm        cmp    dword ptr [ebp-0x34], 0;
	__asm        je     _T5d;
// LINE 1188:
_T35:
	mp.id = 0xffffffff;
// LINE 1189:
	mp.op = 0x27;
// LINE 1190:
	mp.flags = 0x0;
// LINE 1191:
	mp.i2num = 0x1;
// LINE 1192:
	S3MissionUpdate(mp.op);
// LINE 1196:
_T5d:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x50], 0;
	__asm        jne    _Tc6;
// LINE 1198:
	__asm        mov    eax, CameraCell.x;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x20];
	__asm        cdq;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        mov    deltaX, eax;
// LINE 1199:
	__asm        mov    eax, CameraCell.y;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x24];
	__asm        cdq;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        mov    deltaY, eax;
// LINE 1200:
	__asm        mov    eax, deltaX;
	__asm        cmp    deltaY, eax;
	__asm        jge    _Tac;
// LINE 1201:
	dist = ((deltaX + deltaX) + deltaY);
// LINE 1202:
	__asm        jmp    _Tb7;
// LINE 1203:
_Tac:
	dist = ((deltaY + deltaY) + deltaX);
// LINE 1206:
_Tb7:
	__asm        cmp    dist, 0x14;
	__asm        jle    _Tc6;
// LINE 1207:
	return;
// LINE 1211:
_Tc6:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+8];
	__asm        shl    eax, 8;
	__asm        lea    eax, [eax+eax*2];
	__asm        mov    ecx, this;
	__asm        add    eax, [ecx+0x70];
	__asm        mov    x, eax;
// LINE 1212:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x10];
	__asm        shl    eax, 8;
	__asm        lea    eax, [eax+eax*2];
	__asm        mov    ecx, this;
	__asm        add    eax, [ecx+0x78];
	__asm        mov    z, eax;
// LINE 1214:
	this->crashLocation.x = ((x + 0x20000000) >> 0x16);
// LINE 1215:
	this->crashLocation.y = ((0x20000000 - z) >> 0x16);
// LINE 1227:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x44];
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x40];
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0x3C], eax;
	__asm        cmp    dword ptr [ebp-0x3C], 0;
	__asm        jne    _T15c;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T161;
_T15c:
	__asm        jmp    _T161;
_T161:
	__asm        jmp    _T166;
_T166:
	__asm        mov    eax, [ebp-0x3C];
	__asm        mov    cptr, eax;
// LINE 1230:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+2];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x70];
	__asm        mov    ecx, this;
	__asm        mov    [ecx+8], eax;
// LINE 1231:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+4];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x74];
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0xC], eax;
// LINE 1232:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+6];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x78];
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0x10], eax;
// LINE 1234:
	this->remainingDist = MTNormalize((this + 0x8));
// LINE 1236:
	__asm        mov    eax, this;
	__asm        movsx  eax, word ptr [eax+0x64];
	__asm        test   al, 1;
	__asm        je     _T1e8;
// LINE 1238:
	MTCreateDOF4x4((this + 0x7c), (this + 0x8));
// LINE 1241:
_T1e8:
	this->flags[2] = 0x0;
// LINE 1242:
	this->flags[3] = 0x1;
// LINE 1244:
	this->smokeTime = 0x0;
// LINE 1247:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T215;

	__asm        jmp    _T229;
_T215:
	__asm        jmp    _T243;

	__asm        jmp    _T229;

	__asm        cmp    dword ptr [ebp-0x38], 0;
	__asm        jne    _T243;
// LINE 1249:
_T229:
	S3DSPlay(0x1d, (0x6c1210 + 0x5c), 0x0);
// LINE 1251:
	__asm        jmp    _T243;
// LINE 1258:
_T243:
	return;
}

// FUNCTION: COPTER_D 0x0052bdad
int32_t PlaneClass::IsPlaneOutOfCameraRange() {
	/*bp-0x4*/   int32_t deltaX;
	/*bp-0x8*/   int32_t deltaY;

// LINE 1281:
	deltaX = (CameraCell.x - this->currentLocation.x);
// LINE 1282:
	deltaY = (CameraCell.y - this->currentLocation.y);
// LINE 1287:
__WHILE_28:
	while ((deltaX > 0x80)) {
		// LINE 1289:
			deltaX -= 0x100;
	}
// LINE 1291:
__WHILE_41:
	while ((deltaX < -0x80)) {
		// LINE 1293:
			deltaX += 0x100;
	}
// LINE 1296:
__WHILE_57:
	while ((deltaY > 0x80)) {
		// LINE 1298:
			deltaY -= 0x100;
	}
// LINE 1300:
__WHILE_70:
	while ((deltaY < -0x80)) {
		// LINE 1302:
			deltaY += 0x100;
	}
// LINE 1305:
_T86:
	__asm        cmp    deltaX, 0;
	__asm        jge    _T98;
// LINE 1307:
	__asm        mov    eax, deltaX;
	__asm        neg    eax;
	__asm        mov    deltaX, eax;
// LINE 1309:
_T98:
	__asm        cmp    deltaY, 0;
	__asm        jge    _Taa;
// LINE 1311:
	__asm        mov    eax, deltaY;
	__asm        neg    eax;
	__asm        mov    deltaY, eax;
// LINE 1315:
_Taa:
	__asm        mov    eax, G_ViewSize;
	__asm        sar    eax, 1;
	__asm        add    eax, 4;
	__asm        cmp    eax, deltaX;
	__asm        jl     _Td2;

	__asm        mov    eax, G_ViewSize;
	__asm        sar    eax, 1;
	__asm        add    eax, 4;
	__asm        cmp    eax, deltaY;
	__asm        jge    _Te1;
// LINE 1317:
_Td2:
	return 0x1;
// LINE 1319:
	__asm        jmp    __RETURN;
// LINE 1321:
_Te1:
	return 0x0;
// LINE 1323:
__RETURN:
}

// FUNCTION: COPTER_D 0x0052be9a
void PlaneClass::SlowDown() {
// LINE 1342:
	this->speed -= 0x20000;
// LINE 1343:
	return;
}

// FUNCTION: COPTER_D 0x0052beba
void PlaneClass::Stop() {
// LINE 1363:
	this->speed = 0x0;
// LINE 1364:
	return;
}

// FUNCTION: COPTER_D 0x0052beda
void PlaneClass::MoveForward() {
	/*bp-0x4*/   int32_t dist;
	/*bp-0x10*/  /*packed*/ struct Point3d newloc; // 0xc bytes
	/*bp-0x18*/  /*packed*/ struct Point2d newpos; // 0x8 bytes
	/*bp-0x1c*/  /*packed*/ struct _CELL_INFO *cptr;
	/*bp-0x20*/  /*packed*/ struct _CELL_INFO *lcptr;

// LINE 1390:
	__asm        mov    eax, LoopTime;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x18];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    dist, eax;
// LINE 1394:
	__asm        jmp    _T29;
_T29:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+7];
	__asm        test   eax, eax;
	__asm        jne    _Ta0;
// LINE 1396:
	__asm        mov    eax, this;
	__asm        mov    ecx, dist;
	__asm        cmp    [eax+0x14], ecx;
	__asm        jge    _T50;
// LINE 1397:
	dist = this->remainingDist;
// LINE 1398:
_T50:
	__asm        xor    eax, eax;
	__asm        sub    eax, dist;
	__asm        neg    eax;
	__asm        mov    ecx, this;
	__asm        sub    [ecx+0x14], eax;
// LINE 1403:
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+8];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        add    [ecx+0x70], eax;
// LINE 1405:
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x10];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        add    [ecx+0x78], eax;
// LINE 1406:
	this->dyObj.loc.y += this->altAdjustment;
// LINE 1408:
	__asm        jmp    _T2bf;
// LINE 1413:
_Ta0:
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+8];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x70];
	__asm        add    ecx, eax;
	__asm        mov    newloc.x, ecx;
// LINE 1414:
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0xC];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x74];
	__asm        add    ecx, eax;
	__asm        mov    newloc.y, ecx;
// LINE 1415:
	__asm        mov    eax, dist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x10];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x78];
	__asm        add    ecx, eax;
	__asm        mov    newloc.z, ecx;
// LINE 1417:
	newpos.x = ((newloc.x + 0x20000000) >> 0x16);
// LINE 1418:
	newpos.y = ((0x20000000 - newloc.z) >> 0x16);
// LINE 1420:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x24];
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x20];
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0x24], eax;
	__asm        cmp    dword ptr [ebp-0x24], 0;
	__asm        jne    _T160;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T165;
_T160:
	__asm        jmp    _T165;
_T165:
	__asm        jmp    _T16a;
_T16a:
	__asm        mov    eax, [ebp-0x24];
	__asm        mov    lcptr, eax;
// LINE 1421:
	__asm        mov    eax, newpos.y;
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, newpos.x;
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0x28], eax;
	__asm        cmp    dword ptr [ebp-0x28], 0;
	__asm        jne    _T1b4;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T1b9;
_T1b4:
	__asm        jmp    _T1b9;
_T1b9:
	__asm        jmp    _T1be;
_T1be:
	__asm        mov    eax, [ebp-0x28];
	__asm        mov    cptr, eax;
// LINE 1424:
	__asm        xor    eax, eax;
	__asm        sub    eax, G_AvLoopTime;
	__asm        neg    eax;
	__asm        mov    ecx, this;
	__asm        sub    [ecx+0x48], eax;
// LINE 1427:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x48], 0;
	__asm        jge    _T1fd;
// LINE 1429:
	S3ExplosionSmokeStart(cptr, newloc.x, 0x1);
// LINE 1430:
	this->smokeTime = 0x3333;
// LINE 1434:
_T1fd:
	this->PlaneClass::PlaneCollisionCheck(dist, lcptr);
// LINE 1435:
	__asm        jmp    _T212;
_T212:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        jne    _T226;
// LINE 1437:
	return;
// LINE 1441:
_T226:
	__asm        mov    eax, lcptr;
	__asm        cmp    cptr, eax;
	__asm        je     _T25b;
// LINE 1443:
	this->PlaneClass::PlaneCollisionCheck(dist, cptr);
// LINE 1444:
	__asm        jmp    _T247;
_T247:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        jne    _T25b;
// LINE 1445:
	return;
// LINE 1456:
_T25b:
	__asm        mov    eax, lcptr;
	__asm        cmp    cptr, eax;
	__asm        je     _T2a6;
// LINE 1458:
	__asm        jmp    _T26c;
_T26c:
	this->PlaneClass::UnlinkFromCell((this + 0x20));
// LINE 1459:
	this->currentLocation.x = newpos.x;
	this->currentLocation.y = newpos.y;
	__asm        jmp    _T292;
// LINE 1460:
_T292:
	__asm        jmp    _T297;
_T297:
	this->PlaneClass::LinkToCell((this + 0x20));
// LINE 1463:
_T2a6:
	__asm        lea    eax, newloc.x;
	__asm        mov    ecx, this;
	__asm        add    ecx, 0x70;
	__asm        mov    edx, [eax];
	__asm        mov    [ecx], edx;
	__asm        mov    edx, [eax+4];
	__asm        mov    [ecx+4], edx;
	__asm        mov    eax, [eax+8];
	__asm        mov    [ecx+8], eax;
// LINE 1465:
_T2bf:
	return;
}

// FUNCTION: COPTER_D 0x0052c1a3
int32_t PlaneClass::PlaneCollisionCheck(int32_t dist, /*packed*/ struct _CELL_INFO *cptr) {
	/*bp-0x4*/   int32_t speed;
	/*bp-0x8*/   /*packed*/ struct Point3d *norm;
	/*bp-0x2c*/  /*packed*/ struct VRObjInfo oinfo; // 0x24 bytes
	/*bp-0x38*/  /*packed*/ struct Point3d sloc; // 0xc bytes
	/*bp-0x44*/  /*packed*/ struct Point3d cloc; // 0xc bytes
	/*bp-0x48*/  /*packed*/ struct _STOBJ_INST *stobj;
	/*bp-0x88*/  int32_t refmat[4][4]; // 0x40 bytes
	/*bp-0x94*/  /*packed*/ struct Point3d vec; // 0xc bytes
	/*bp-0xa0*/  /*packed*/ struct Point3d center; // 0xc bytes
	/*bp-0xa8*/  /*packed*/ struct Point2d celloc; // 0x8 bytes
	/*bp-0xb4*/  /*packed*/ struct Point3d loc; // 0xc bytes
	/*bp-0xcc*/  /*packed*/ struct _MISSION_PARMS mp; // 0x18 bytes
	/*bp-0xd0*/  long j;
	/*bp-0xdc*/  /*packed*/ struct Point3d oloc; // 0xc bytes
	/*bp-0xe0*/  int32_t newdist;
	/*bp-0xe4*/  long num_debris;
	/*bp-0xe8*/  long new_mission_id;

// LINE 1490:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+2];
	__asm        shl    eax, 0x10;
	__asm        mov    cloc.x, eax;
// LINE 1491:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+4];
	__asm        shl    eax, 0x10;
	__asm        mov    cloc.y, eax;
// LINE 1492:
	__asm        mov    eax, cptr;
	__asm        movsx  eax, word ptr [eax+6];
	__asm        shl    eax, 0x10;
	__asm        mov    cloc.z, eax;
// LINE 1493:
	celloc.x = ((cloc.x + 0x20000000) >> 0x16);
// LINE 1494:
	celloc.y = ((0x20000000 - cloc.z) >> 0x16);
// LINE 1497:
	stobj = cptr->stptr;
// LINE 1498:
__WHILE_64:
	while ((stobj != 0x0)) {
		// LINE 1502:
			_VRGetObjInfo(stobj->mesh, oinfo.Faces);
		// LINE 1503:
			_VRGetObjCenter(stobj->mesh, center.x);
		// LINE 1505:
			__asm        lea    eax, cloc.x;
			__asm        lea    ecx, oloc.x;
			__asm        mov    edx, [eax];
			__asm        mov    [ecx], edx;
			__asm        mov    edx, [eax+4];
			__asm        mov    [ecx+4], edx;
			__asm        mov    eax, [eax+8];
			__asm        mov    [ecx+8], eax;
		// LINE 1506:
			oloc.x += center.x;
		// LINE 1507:
			oloc.y += center.y;
		// LINE 1508:
			oloc.z += center.z;
		// LINE 1514:
			newdist = S3MissileSphereHit((this + 0x70), (this + 0x8), dist, oloc.x, oinfo.Radius);
		// LINE 1518:
			__asm        cmp    newdist, 0;
			__asm        jle    _T147;
		// LINE 1525:
			newdist = VRStObjPolyHit((this + 0x70), (this + 0x8), dist, cloc.x, stobj->mesh, norm);
		// LINE 1528:
		_T147:
			__asm        cmp    newdist, 0;
			__asm        jle    _T5d6;
		// LINE 1532:
			__asm        mov    eax, newdist;
			__asm        push   eax;
			__asm        mov    eax, this;
			__asm        mov    eax, [eax+8];
			__asm        push   eax;
			__asm        call   _FixedMul;
			__asm        add    esp, 8;
			__asm        mov    ecx, this;
			__asm        mov    ecx, [ecx+0x70];
			__asm        add    ecx, eax;
			__asm        mov    loc.x, ecx;
		// LINE 1533:
			__asm        mov    eax, newdist;
			__asm        push   eax;
			__asm        mov    eax, this;
			__asm        mov    eax, [eax+0xC];
			__asm        push   eax;
			__asm        call   _FixedMul;
			__asm        add    esp, 8;
			__asm        mov    ecx, this;
			__asm        mov    ecx, [ecx+0x74];
			__asm        add    ecx, eax;
			__asm        mov    loc.y, ecx;
		// LINE 1534:
			__asm        mov    eax, newdist;
			__asm        push   eax;
			__asm        mov    eax, this;
			__asm        mov    eax, [eax+0x10];
			__asm        push   eax;
			__asm        call   _FixedMul;
			__asm        add    esp, 8;
			__asm        mov    ecx, this;
			__asm        mov    ecx, [ecx+0x78];
			__asm        add    ecx, eax;
			__asm        mov    loc.z, ecx;
		// LINE 1535:
			__asm        lea    eax, loc.x;
			__asm        lea    ecx, sloc.x;
			__asm        mov    edx, [eax];
			__asm        mov    [ecx], edx;
			__asm        mov    edx, [eax+4];
			__asm        mov    [ecx+4], edx;
			__asm        mov    eax, [eax+8];
			__asm        mov    [ecx+8], eax;
		// LINE 1536:
			__asm        xor    eax, eax;
			__asm        sub    eax, cloc.x;
			__asm        neg    eax;
			__asm        sub    loc.x, eax;
		// LINE 1537:
			__asm        xor    eax, eax;
			__asm        sub    eax, cloc.y;
			__asm        neg    eax;
			__asm        sub    loc.y, eax;
		// LINE 1538:
			__asm        xor    eax, eax;
			__asm        sub    eax, cloc.z;
			__asm        neg    eax;
			__asm        sub    loc.z, eax;
		// LINE 1540:
			S3ExplosionStart(cptr, loc.x, loc.y, loc.z, 0x4, this->missionId);
		// LINE 1542:
			S3DSPlay(0x1a, sloc.x, 0x0);
		// LINE 1545:
			this->PlaneClass::UnPlacePlane();
		// LINE 1546:
			this->flags[3] = 0x0;
		// LINE 1549:
			new_mission_id = 0xffffffff;
		// LINE 1551:
			__asm        mov    eax, celloc.y;
			__asm        push   eax;
			__asm        mov    eax, celloc.x;
			__asm        push   eax;
			__asm        call   S3FireCanCellBurn;
			__asm        add    esp, 8;
			__asm        cmp    eax, 1;
			__asm        jne    _T2e4;

			__asm        mov    eax, cptr;
			__asm        movsx  eax, word ptr [eax];
			__asm        test   al, 0x20;
			__asm        jne    _T2e4;
		// LINE 1554:
			__asm        call   S3FireGetCount;
			__asm        test   eax, eax;
			__asm        jne    _T2cb;
		// LINE 1556:
			new_mission_id = S3MissionStart(celloc.x, celloc.y, 0x1);
		// LINE 1558:
			__asm        jmp    _T2e4;
		// LINE 1560:
		_T2cb:
			new_mission_id = S3FireAddToNearest(cptr, celloc.x);
		// LINE 1564:
		_T2e4:
			__asm        cmp    new_mission_id, 0xFFFFFFFF;
			__asm        je     _T449;
		// LINE 1568:
			mp.op = 0x1d;
		// LINE 1569:
			mp.i2num = 0x4;
		// LINE 1570:
			mp.id = this->missionId;
		// LINE 1571:
			S3MissionUpdate(mp.op);
		// LINE 1572:
			this->missionId = new_mission_id;
		// LINE 1574:
			__asm        call   rand;
			__asm        mov    ecx, cptr;
			__asm        movsx  ecx, word ptr [ecx+8];
			__asm        movsx  eax, ax;
			__asm        cdq;
			__asm        idiv   ecx;
			__asm        lea    eax, [edx+3];
			__asm        mov    num_debris, eax;
		// LINE 1575:
		_FOR_35c:
			for (j = 0x0; (j < num_debris); j++) {
				// LINE 1577:
					__asm        call   rand;
					__asm        mov    ecx, 0x1E;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        mov    eax, edx;
					__asm        shl    eax, 0x10;
					__asm        add    eax, 0x190000;
					__asm        mov    speed, eax;
				// LINE 1578:
					_VRIdentity4x4(refmat[0][0]);
				// LINE 1579:
					__asm        lea    eax, refmat[0][0];
					__asm        push   eax;
					__asm        call   rand;
					__asm        mov    ecx, 0xE10;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        mov    eax, edx;
					__asm        shl    eax, 0x10;
					__asm        push   eax;
					__asm        call   _VRPostYRot;
					__asm        add    esp, 8;
				// LINE 1580:
					__asm        lea    eax, refmat[0][0];
					__asm        push   eax;
					__asm        call   rand;
					__asm        mov    ecx, 0x78;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        lea    eax, [edx+0x2EE];
					__asm        shl    eax, 0x10;
					__asm        push   eax;
					__asm        call   _VRPostXRot;
					__asm        add    esp, 8;
				// LINE 1581:
					_VRRotateP3d(0x59b518, vec.x, refmat[0][0]);
				// LINE 1592:
					S3MissileStart(0x4, celloc.x, sloc.x, vec.x, 0x8, (this + 0x58), speed, this->missionId);
			}
		// LINE 1595:
		_T444:
			__asm        jmp    _T5cc;
		// LINE 1598:
		_T449:
			__asm        call   rand;
			__asm        mov    ecx, cptr;
			__asm        movsx  ecx, word ptr [ecx+8];
			__asm        movsx  eax, ax;
			__asm        cdq;
			__asm        idiv   ecx;
			__asm        lea    eax, [edx+3];
			__asm        mov    num_debris, eax;
		// LINE 1599:
		_FOR_473:
			for (j = 0x0; (j < num_debris); j++) {
				// LINE 1601:
					__asm        call   rand;
					__asm        mov    ecx, 0x1E;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        mov    eax, edx;
					__asm        shl    eax, 0x10;
					__asm        add    eax, 0x190000;
					__asm        mov    speed, eax;
				// LINE 1602:
					_VRIdentity4x4(refmat[0][0]);
				// LINE 1603:
					__asm        lea    eax, refmat[0][0];
					__asm        push   eax;
					__asm        call   rand;
					__asm        mov    ecx, 0xE10;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        mov    eax, edx;
					__asm        shl    eax, 0x10;
					__asm        push   eax;
					__asm        call   _VRPostYRot;
					__asm        add    esp, 8;
				// LINE 1604:
					__asm        lea    eax, refmat[0][0];
					__asm        push   eax;
					__asm        call   rand;
					__asm        mov    ecx, 0x78;
					__asm        movsx  eax, ax;
					__asm        cdq;
					__asm        idiv   ecx;
					__asm        lea    eax, [edx+0x2EE];
					__asm        shl    eax, 0x10;
					__asm        push   eax;
					__asm        call   _VRPostXRot;
					__asm        add    esp, 8;
				// LINE 1605:
					_VRRotateP3d(0x59b518, vec.x, refmat[0][0]);
				// LINE 1616:
					S3MissileStart(0x4, celloc.x, sloc.x, vec.x, 0x8, (this + 0x58), speed, this->missionId);
			}
		// LINE 1621:
		_T55b:
			mp.op = 0x1d;
		// LINE 1622:
			mp.i2num = 0x0;
		// LINE 1623:
			mp.id = this->missionId;
		// LINE 1624:
			S3MissionUpdate(mp.op);
		// LINE 1626:
			mp.op = 0x0;
		// LINE 1627:
			mp.id = this->missionId;
		// LINE 1628:
			__asm        mov    eax, celloc.x;
			__asm        mov    ecx, celloc.y;
			__asm        lea    edx, mp.maploc.x;
			__asm        mov    [edx], eax;
			__asm        mov    [edx+4], ecx;
		// LINE 1629:
			S3MissionUpdate(mp.op);
		// LINE 1635:
		_T5cc:
			return 0x1;
		// LINE 1640:
		_T5d6:
			stobj = stobj->next;
	}
// LINE 1647:
_T5e3:
	newdist = S3MissileGroundHit((this + 0x70), (this + 0x8), dist, cloc.x);
// LINE 1648:
	__asm        cmp    newdist, 0;
	__asm        jle    _T878;
// LINE 1652:
	__asm        mov    eax, newdist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+8];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x70];
	__asm        add    ecx, eax;
	__asm        mov    loc.x, ecx;
// LINE 1653:
	__asm        mov    eax, newdist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0xC];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x74];
	__asm        add    ecx, eax;
	__asm        mov    loc.y, ecx;
// LINE 1654:
	__asm        mov    eax, newdist;
	__asm        push   eax;
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x10];
	__asm        push   eax;
	__asm        call   _FixedMul;
	__asm        add    esp, 8;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x78];
	__asm        add    ecx, eax;
	__asm        mov    loc.z, ecx;
// LINE 1655:
	__asm        lea    eax, loc.x;
	__asm        lea    ecx, sloc.x;
	__asm        mov    edx, [eax];
	__asm        mov    [ecx], edx;
	__asm        mov    edx, [eax+4];
	__asm        mov    [ecx+4], edx;
	__asm        mov    eax, [eax+8];
	__asm        mov    [ecx+8], eax;
// LINE 1656:
	__asm        xor    eax, eax;
	__asm        sub    eax, cloc.x;
	__asm        neg    eax;
	__asm        sub    loc.x, eax;
// LINE 1657:
	__asm        xor    eax, eax;
	__asm        sub    eax, cloc.y;
	__asm        neg    eax;
	__asm        sub    loc.y, eax;
// LINE 1658:
	__asm        xor    eax, eax;
	__asm        sub    eax, cloc.z;
	__asm        neg    eax;
	__asm        sub    loc.z, eax;
// LINE 1662:
	S3ExplosionStart(cptr, loc.x, loc.y, loc.z, 0x4, this->missionId);
// LINE 1664:
	S3DSPlay(0x1a, sloc.x, 0x0);
// LINE 1667:
	this->PlaneClass::UnPlacePlane();
// LINE 1668:
	this->flags[3] = 0x0;
// LINE 1710:
	__asm        call   rand;
	__asm        mov    ecx, cptr;
	__asm        movsx  ecx, word ptr [ecx+8];
	__asm        movsx  eax, ax;
	__asm        cdq;
	__asm        idiv   ecx;
	__asm        lea    eax, [edx+3];
	__asm        mov    num_debris, eax;
// LINE 1711:
_FOR_754:
	for (j = 0x0; (j < num_debris); j++) {
		// LINE 1713:
			__asm        call   rand;
			__asm        mov    ecx, 0x1E;
			__asm        movsx  eax, ax;
			__asm        cdq;
			__asm        idiv   ecx;
			__asm        mov    eax, edx;
			__asm        shl    eax, 0x10;
			__asm        add    eax, 0x190000;
			__asm        mov    speed, eax;
		// LINE 1714:
			_VRIdentity4x4(refmat[0][0]);
		// LINE 1715:
			__asm        lea    eax, refmat[0][0];
			__asm        push   eax;
			__asm        call   rand;
			__asm        mov    ecx, 0xE10;
			__asm        movsx  eax, ax;
			__asm        cdq;
			__asm        idiv   ecx;
			__asm        mov    eax, edx;
			__asm        shl    eax, 0x10;
			__asm        push   eax;
			__asm        call   _VRPostYRot;
			__asm        add    esp, 8;
		// LINE 1716:
			__asm        lea    eax, refmat[0][0];
			__asm        push   eax;
			__asm        call   rand;
			__asm        mov    ecx, 0x78;
			__asm        movsx  eax, ax;
			__asm        cdq;
			__asm        idiv   ecx;
			__asm        lea    eax, [edx+0x2EE];
			__asm        shl    eax, 0x10;
			__asm        push   eax;
			__asm        call   _VRPostXRot;
			__asm        add    esp, 8;
		// LINE 1717:
			_VRRotateP3d(0x59b518, vec.x, refmat[0][0]);
		// LINE 1728:
			S3MissileStart(0x4, celloc.x, sloc.x, vec.x, 0x8, (this + 0x58), speed, this->missionId);
	}
// LINE 1733:
_T83c:
	mp.op = 0x1d;
// LINE 1734:
	mp.i2num = 0x0;
// LINE 1735:
	mp.id = this->missionId;
// LINE 1736:
	S3MissionUpdate(mp.op);
// LINE 1740:
	return 0x1;
// LINE 1743:
_T878:
	return 0x0;
// LINE 1744:
}

// FUNCTION: COPTER_D 0x0052ca29
int32_t PlaneClass::HaveIReachedNextLoc() {
// LINE 1765:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x14], 0;
	__asm        jg     _T28;
// LINE 1767:
	return 0x1;
// LINE 1769:
	__asm        jmp    __RETURN;
// LINE 1771:
_T28:
	return 0x0;
// LINE 1773:
__RETURN:
}

// FUNCTION: COPTER_D 0x0052ca5d
int32_t PlaneClass::AmIInANewCell() {
// LINE 1797:
	this->currentCell.x = ((this->dyObj.loc.x + 0x20000000) >> 0x16);
// LINE 1798:
	this->currentCell.y = ((0x20000000 - this->dyObj.loc.z) >> 0x16);
// LINE 1802:
	__asm        mov    eax, this;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x20];
	__asm        cmp    [eax+0x28], ecx;
	__asm        jne    _T58;

	__asm        mov    eax, this;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x24];
	__asm        cmp    [eax+0x2C], ecx;
	__asm        je     _T62;
// LINE 1804:
_T58:
	return 0x1;
// LINE 1807:
_T62:
	return 0x0;
// LINE 1808:
}

// FUNCTION: COPTER_D 0x0052cacb
void PlaneClass::UnlinkFromCell(const /*packed*/ struct Point2d& point) {
	/*bp-0x4*/   /*packed*/ struct _CELL_INFO *cellPointer;

// LINE 1830:
	__asm        mov    eax, point;
	__asm        mov    eax, [eax+4];
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, point;
	__asm        mov    ecx, [ecx];
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0xC], eax;
	__asm        cmp    dword ptr [ebp-0xC], 0;
	__asm        jne    _T55;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T5a;
_T55:
	__asm        jmp    _T5a;
_T5a:
	__asm        jmp    _T5f;
_T5f:
	__asm        mov    eax, [ebp-0xC];
	__asm        mov    cellPointer, eax;
// LINE 1832:
	__asm        cmp    cellPointer, 0;
	__asm        jne    _T8b;

	_assert(0x5b7700, 0x5b76dc, 0x728);
	__asm        jmp    _T90;
_T8b:
	__asm        jmp    _T90;
// LINE 1834:
_T90:
	__asm        cmp    cellPointer, 0;
	__asm        je     _T10a;
// LINE 1836:
// Block start:
	/*bp-0x8*/   /*packed*/ struct _DYOBJ_INST **dyptrptr;
	dyptrptr = (cellPointer + 0x10);
// LINE 1838:
__WHILE_a3:
	while ((dyptrptr-> != 0x0)) {
		// LINE 1842:
			__asm        mov    eax, this;
			__asm        add    eax, 0x58;
			__asm        mov    ecx, dyptrptr;
			__asm        cmp    eax, [ecx];
			__asm        jne    _Td0;
		// LINE 1846:
			dyptrptr-> = this->dyObj.next;
		// LINE 1848:
			return;
		// LINE 1851:
		_Td0:
			dyptrptr = dyptrptr->;
	}
// LINE 1854:
_Tdd:
	__asm        mov    eax, dyptrptr;
	__asm        cmp    dword ptr [eax], 0;
	__asm        jne    _T105;

	_assert(0x5b7730, 0x5b770c, 0x73e);
	__asm        jmp    _T10a;
_T105:
	__asm        jmp    _T10a;
// LINE 1856:
// Block end:
_T10a:
	return;
}

// FUNCTION: COPTER_D 0x0052cbe1
void PlaneClass::LinkToCell(const /*packed*/ struct Point2d& point) {
	/*bp-0x4*/   /*packed*/ struct _CELL_INFO *cellPointer;

// LINE 1878:
	__asm        mov    eax, point;
	__asm        mov    eax, [eax+4];
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, point;
	__asm        mov    ecx, [ecx];
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-8], eax;
	__asm        cmp    dword ptr [ebp-8], 0;
	__asm        jne    _T55;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T5a;
_T55:
	__asm        jmp    _T5a;
_T5a:
	__asm        jmp    _T5f;
_T5f:
	__asm        mov    eax, [ebp-8];
	__asm        mov    cellPointer, eax;
// LINE 1880:
	__asm        mov    eax, cellPointer;
	__asm        mov    ecx, this;
	__asm        add    ecx, 0x58;
	__asm        cmp    [eax+0x10], ecx;
	__asm        jne    _T93;

	_assert(0x5b7760, 0x5b773c, 0x758);
	__asm        jmp    _T98;
_T93:
	__asm        jmp    _T98;
// LINE 1882:
_T98:
	this->dyObj.next = cellPointer->dyptr;
// LINE 1883:
	cellPointer->dyptr = (this + 0x58);
// LINE 1884:
	return;
}

// FUNCTION: COPTER_D 0x0052cc9d
void PlaneClass::AdjustCurrentPosition() {
	/*bp-0x4*/   int32_t alt;
	/*bp-0x8*/   /*packed*/ struct _CELL_INFO *cellPointer;

// LINE 1905:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x24];
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, this;
	__asm        mov    ecx, [ecx+0x20];
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0xC], eax;
	__asm        cmp    dword ptr [ebp-0xC], 0;
	__asm        jne    _T56;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T5b;
_T56:
	__asm        jmp    _T5b;
_T5b:
	__asm        jmp    _T60;
_T60:
	__asm        mov    eax, [ebp-0xC];
	__asm        mov    cellPointer, eax;
// LINE 1907:
	__asm        cmp    cellPointer, 0;
	__asm        jne    _T8c;

	_assert(0x5b77a4, 0x5b7780, 0x773);
	__asm        jmp    _T91;
_T8c:
	__asm        jmp    _T91;
// LINE 1911:
_T91:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+2];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0x70], eax;
// LINE 1912:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+4];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0x74], eax;
// LINE 1913:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+6];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0x78], eax;
// LINE 1916:
	alt = S3HeliHighestBuildAlt(cellPointer);
// LINE 1917:
	__asm        cmp    alt, 0x15E0000;
	__asm        jge    _Tec;
// LINE 1918:
	this->dyObj.loc.y += 0x17c0000;
// LINE 1919:
	__asm        jmp    _Tfa;
// LINE 1920:
_Tec:
	this->dyObj.loc.y += (alt + 0x1e0000);
// LINE 1923:
_Tfa:
	return;
}

// FUNCTION: COPTER_D 0x0052cda1
void PlaneClass::AdjustNextPosition() {
	/*bp-0x40*/  int32_t mat[4][4]; // 0x40 bytes
	/*bp-0x4c*/  /*packed*/ struct Point3d nextFineLocation; // 0xc bytes
	/*bp-0x50*/  /*packed*/ struct _CELL_INFO *cellPointer;

// LINE 1944:
	__asm        mov    eax, CameraCell.y;
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, CameraCell.x;
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0x54], eax;
	__asm        cmp    dword ptr [ebp-0x54], 0;
	__asm        jne    _T55;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T5a;
_T55:
	__asm        jmp    _T5a;
_T5a:
	__asm        jmp    _T5f;
_T5f:
	__asm        mov    eax, [ebp-0x54];
	__asm        mov    cellPointer, eax;
// LINE 1948:
	__asm        cmp    cellPointer, 0;
	__asm        jne    _T8b;

	_assert(0x5b77d4, 0x5b77b0, 0x79c);
	__asm        jmp    _T90;
_T8b:
	__asm        jmp    _T90;
// LINE 1951:
_T90:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+2];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x70];
	__asm        mov    nextFineLocation.x, eax;
// LINE 1952:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+6];
	__asm        shl    eax, 0x10;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x78];
	__asm        mov    nextFineLocation.z, eax;
// LINE 1953:
	nextFineLocation.y = 0x0;
// LINE 1956:
	MTNormalize(nextFineLocation.x);
// LINE 1959:
	_VRIdentity4x4(mat[0][0]);
// LINE 1960:
	__asm        lea    eax, mat[0][0];
	__asm        push   eax;
	__asm        mov    ebx, 0x12C0000;
	__asm        call   rand;
	__asm        mov    ecx, 0x258;
	__asm        movsx  eax, ax;
	__asm        cdq;
	__asm        idiv   ecx;
	__asm        mov    eax, edx;
	__asm        shl    eax, 0x10;
	__asm        sub    ebx, eax;
	__asm        push   ebx;
	__asm        call   _VRPostYRot;
	__asm        add    esp, 8;
// LINE 1961:
	_VRRotateP3d(nextFineLocation.x, (this + 0x8), mat[0][0]);
// LINE 1963:
	__asm        mov    eax, this;
	__asm        movsx  eax, word ptr [eax+0x64];
	__asm        test   al, 1;
	__asm        je     _T13a;
// LINE 1965:
	MTCreateDOF4x4((this + 0x7c), (this + 0x8));
// LINE 1967:
_T13a:
	return;
}

// FUNCTION: COPTER_D 0x0052cee5
void PlaneClass::AdjustNextAltitude() {
	/*bp-0x40*/  int32_t mat[4][4]; // 0x40 bytes
	/*bp-0x4c*/  /*packed*/ struct Point3d loc; // 0xc bytes
	/*bp-0x50*/  int32_t altdiff;
	/*bp-0x54*/  int32_t alt;
	/*bp-0x5c*/  /*packed*/ struct Point2d nextcell; // 0x8 bytes
	/*bp-0x60*/  /*packed*/ struct _CELL_INFO *cellPointer;

// LINE 1995:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T26;

	__asm        jmp    _Tb2;

	__asm        jmp    _T35;
_T26:
	__asm        jmp    _T35;

	__asm        cmp    dword ptr [ebp-0x64], 0;
	__asm        je     _Tb2;
// LINE 1997:
_T35:
	__asm        call   rand;
	__asm        movsx  eax, ax;
	__asm        cdq;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        and    eax, 7;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        jne    _Tb2;
// LINE 1999:
	_VRIdentity4x4(mat[0][0]);
// LINE 2000:
	__asm        lea    eax, mat[0][0];
	__asm        push   eax;
	__asm        mov    ebx, 0x320;
	__asm        call   rand;
	__asm        mov    ecx, 0x640;
	__asm        movsx  eax, ax;
	__asm        cdq;
	__asm        idiv   ecx;
	__asm        sub    ebx, edx;
	__asm        shl    ebx, 0x10;
	__asm        push   ebx;
	__asm        call   _VRPostYRot;
	__asm        add    esp, 8;
// LINE 2001:
	_VRRotateP3d((this + 0x8), loc.x, mat[0][0]);
// LINE 2002:
	__asm        lea    eax, loc.x;
	__asm        mov    ecx, this;
	__asm        add    ecx, 8;
	__asm        mov    edx, [eax];
	__asm        mov    [ecx], edx;
	__asm        mov    edx, [eax+4];
	__asm        mov    [ecx+4], edx;
	__asm        mov    eax, [eax+8];
	__asm        mov    [ecx+8], eax;
// LINE 2008:
_Tb2:
	loc.x = (this->dyObj.loc.x + (this->directionVector.x << 0x5));
// LINE 2009:
	loc.z = ((this->directionVector.z << 0x5) + this->dyObj.loc.z);
// LINE 2011:
	nextcell.x = ((loc.x + 0x20000000) >> 0x16);
// LINE 2012:
	nextcell.y = ((0x20000000 - loc.z) >> 0x16);
// LINE 2014:
	__asm        mov    eax, nextcell.y;
	__asm        and    eax, 0xFF;
	__asm        mov    ecx, nextcell.x;
	__asm        and    ecx, 0xFF;
	__asm        shl    ecx, 0xA;
	__asm        mov    eax, G_omap[0][0][ecx+eax*4];
	__asm        mov    [ebp-0x68], eax;
	__asm        cmp    dword ptr [ebp-0x68], 0;
	__asm        jne    _T138;

	_assert(0x5b57b8, 0x5b780c, 0xd0);
	__asm        jmp    _T13d;
_T138:
	__asm        jmp    _T13d;
_T13d:
	__asm        jmp    _T142;
_T142:
	__asm        mov    eax, [ebp-0x68];
	__asm        mov    cellPointer, eax;
// LINE 2016:
	alt = S3HeliHighestBuildAlt(cellPointer);
// LINE 2017:
	__asm        cmp    alt, 0x15E0000;
	__asm        jge    _T16b;
// LINE 2018:
	alt = 0x15e0000;
// LINE 2023:
_T16b:
	__asm        mov    eax, cellPointer;
	__asm        movsx  eax, word ptr [eax+4];
	__asm        shl    eax, 0x10;
	__asm        add    eax, alt;
	__asm        add    eax, 0x1E0000;
	__asm        mov    ecx, this;
	__asm        sub    eax, [ecx+0x74];
	__asm        mov    altdiff, eax;
// LINE 2024:
	this->altAdjustment = (altdiff >> 0x4);
// LINE 2029:
	this->remainingDist = 0x200000;
// LINE 2034:
	return;
}

// FUNCTION: COPTER_D 0x0052d08b
void PlaneClass::BeamToWithinCameraRange() {
	/*bp-0x4*/   int32_t xdir;
	/*bp-0x8*/   int32_t ydir;
	/*bp-0xc*/   int32_t curr_dist;
	/*bp-0x10*/  int32_t spiral_dist;
	/*bp-0x1c*/  /*packed*/ struct Point3d vec; // 0xc bytes
	/*bp-0x24*/  /*packed*/ struct Point2d beampoint; // 0x8 bytes
	/*bp-0x28*/  int32_t x;
	/*bp-0x2c*/  int32_t y;
	/*bp-0x30*/  int32_t i;
	/*bp-0x34*/  int32_t currentFlag;
	/*bp-0x38*/  int32_t curr_dir;
	/*bp-0x3c*/  int32_t stop_now;
	/*bp-0x40*/  int32_t foundcell;

// LINE 2057:
	curr_dist = 0x0;
// LINE 2058:
	curr_dir = 0xffffffff;
// LINE 2060:
	stop_now = 0x0;
// LINE 2061:
	spiral_dist = 0x7;
// LINE 2063:
	foundcell = 0x0;
// LINE 2067:
	__asm        mov    eax, 0x6C1210;
	__asm        add    eax, 0x14;
	__asm        lea    ecx, vec.x;
	__asm        mov    edx, [eax];
	__asm        mov    [ecx], edx;
	__asm        mov    edx, [eax+4];
	__asm        mov    [ecx+4], edx;
	__asm        mov    eax, [eax+8];
	__asm        mov    [ecx+8], eax;
// LINE 2069:
	this->beamTimer = 0x0;
// LINE 2075:
	__asm        call   rand;
	__asm        movsx  eax, ax;
	__asm        cdq;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        and    eax, 7;
	__asm        xor    eax, edx;
	__asm        sub    eax, edx;
	__asm        mov    [ebp-0x4C], eax;
	__asm        jmp    _T14d;
// LINE 2079:
_T70:
	vec.y = vec.z;
// LINE 2080:
	__asm        mov    eax, vec.x;
	__asm        neg    eax;
	__asm        mov    vec.z, eax;
// LINE 2081:
	vec.x = vec.y;
// LINE 2083:
	vec.x = (vec.x * (G_ViewSize >> 0x1));
// LINE 2084:
	vec.z = (vec.z * (G_ViewSize >> 0x1));
// LINE 2085:
	__asm        jmp    _T170;
// LINE 2088:
_Tad:
	__asm        mov    eax, vec.z;
	__asm        neg    eax;
	__asm        mov    vec.y, eax;
// LINE 2089:
	vec.z = vec.x;
// LINE 2090:
	vec.x = vec.y;
// LINE 2092:
	vec.x = (vec.x * (G_ViewSize >> 0x1));
// LINE 2093:
	vec.z = (vec.z * (G_ViewSize >> 0x1));
// LINE 2094:
	__asm        jmp    _T170;
// LINE 2097:
_Tea:
	__asm        mov    eax, vec.x;
	__asm        neg    eax;
	__asm        mov    vec.x, eax;
// LINE 2098:
	__asm        mov    eax, vec.z;
	__asm        neg    eax;
	__asm        mov    vec.z, eax;
// LINE 2100:
	vec.x = (vec.x * (G_ViewSize >> 0x1));
// LINE 2101:
	vec.z = (vec.z * (G_ViewSize >> 0x1));
// LINE 2102:
	__asm        jmp    _T170;
// LINE 2106:
_T123:
	__asm        mov    eax, G_ViewSize;
	__asm        sar    eax, 1;
	__asm        inc    eax;
	__asm        imul   eax, vec.x;
	__asm        mov    vec.x, eax;
// LINE 2107:
	__asm        mov    eax, G_ViewSize;
	__asm        sar    eax, 1;
	__asm        inc    eax;
	__asm        imul   eax, vec.z;
	__asm        mov    vec.z, eax;
// LINE 2108:
	__asm        jmp    _T170;
// LINE 2109:
	__asm        jmp    _T170;
_T14d:
	__asm        cmp    dword ptr [ebp-0x4C], 0;
	__asm        je     _T70;

	__asm        cmp    dword ptr [ebp-0x4C], 1;
	__asm        je     _Tad;

	__asm        cmp    dword ptr [ebp-0x4C], 2;
	__asm        je     _Tea;

	__asm        jmp    _T123;
// LINE 2111:
_T170:
	__asm        shl    vec.x, 6;
// LINE 2112:
	__asm        shl    vec.z, 6;
// LINE 2114:
	x = (((ViewState.world_pos.x + vec.x) + 0x20000000) >> 0x16);
// LINE 2115:
	y = ((0x20000000 - (ViewState.world_pos.z + vec.z)) >> 0x16);
// LINE 2118:
	__asm        cmp    x, 3;
	__asm        jl     _T1c9;

	__asm        cmp    x, 0x7D;
	__asm        jg     _T1c9;

	__asm        cmp    y, 3;
	__asm        jl     _T1c9;

	__asm        cmp    y, 0x7D;
	__asm        jle    _LOOP_1ce;
// LINE 2119:
_T1c9:
	return;
// LINE 2126:
_LOOP_1ce:
	for (;;) {
		// LINE 2126:
		_LOOP_1ce:
			curr_dir++;
		// LINE 2127:
			__asm        mov    eax, curr_dir;
			__asm        mov    [ebp-0x50], eax;
			__asm        jmp    _T23a;
		// LINE 2131:
		_T1dc:
			curr_dir = 0x0;
		// LINE 2132:
			curr_dist++;
		// LINE 2133:
			xdir = 0x0;
		// LINE 2134:
			ydir = 0xffffffff;
		// LINE 2135:
			__asm        jmp    _T262;
		// LINE 2137:
		_T1f9:
			xdir = 0x1;
		// LINE 2138:
			ydir = 0x0;
		// LINE 2139:
			__asm        jmp    _T262;
		// LINE 2141:
		_T20c:
			curr_dist++;
		// LINE 2142:
			xdir = 0x0;
		// LINE 2143:
			ydir = 0x1;
		// LINE 2144:
			__asm        jmp    _T262;
		// LINE 2146:
		_T222:
			xdir = 0xffffffff;
		// LINE 2147:
			ydir = 0x0;
		// LINE 2148:
			__asm        jmp    _T262;
		// LINE 2149:
			__asm        jmp    _T262;
		_T23a:
			__asm        cmp    dword ptr [ebp-0x50], 4;
			__asm        ja     _T262;

			__asm        mov    eax, [ebp-0x50];
			__asm        jmp    _Switch_24e[0][eax*4];
		// Switch pointers:
		//   _T1dc
		//   _T1f9
		//   _T20c
		//   _T222
		//   _T1dc
		// LINE 2153:
		_T262:
			__asm        mov    eax, curr_dist;
			__asm        cmp    spiral_dist, eax;
			__asm        jne    _T278;
		// LINE 2155:
			curr_dist--;
		// LINE 2156:
			stop_now = 0x1;
		// LINE 2160:
		_T278:
			i = 0x0;
			__asm        jmp    _T287;
		_T284:
			i++;
		_T287:
			__asm        mov    eax, i;
			__asm        cmp    curr_dist, eax;
			__asm        jle    _T2ae;
		// LINE 2162:
			beampoint.x = x;
		// LINE 2163:
			beampoint.y = y;
		// LINE 2164:
			__asm        jmp    _T37c;

			__asm        jmp    _FOR_2d3;
		// LINE 2167:
			__asm        jmp    _T284;
		// LINE 2169:
		_T2ae:
			__asm        cmp    stop_now, 1;
			__asm        jne    _T2bd;
		// LINE 2171:
			__asm        jmp    _T2c2;
		// LINE 2173:
		_T2bd:
	}
// LINE 2176:
_T2c2:
	return;
// LINE 2182:
_FOR_2d3:
	for (currentFlag = 0x3; (currentFlag < 0x4); currentFlag++) {
		// LINE 2184:
			__asm        mov    eax, currentFlag;
			__asm        mov    ecx, this;
			__asm        mov    byte ptr [eax+ecx+4], 0;
	}
// LINE 2187:
_T2f0:
	this->speed = 0x0;
// LINE 2190:
	this->flags[1] = 0x1;
// LINE 2195:
	this->currentLocation.x = beampoint.x;
	this->currentLocation.y = beampoint.y;
	__asm        jmp    _T318;
// LINE 2199:
_T318:
	__asm        jmp    _T31d;
_T31d:
	this->PlaneClass::LinkToCell((this + 0x20));
// LINE 2203:
	this->PlaneClass::AdjustCurrentPosition();
// LINE 2204:
	this->PlaneClass::AdjustNextPosition();
// LINE 2205:
	this->PlaneClass::AdjustNextAltitude();
// LINE 2208:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T359;

	__asm        jmp    _T36d;
_T359:
	__asm        jmp    _T377;

	__asm        jmp    _T36d;

	__asm        cmp    dword ptr [ebp-0x44], 0;
	__asm        jne    _T377;
// LINE 2210:
_T36d:
	this->missileHits = 0x0;
// LINE 2213:
_T377:
	return;
_T37c:
	__asm        jmp    _FOR_2d3;
}

// FUNCTION: COPTER_D 0x0052d411
void PlaneClass::UnPlacePlane() {
// LINE 2236:
	__asm        jmp    _T11;
_T11:
	__asm        mov    eax, this;
	__asm        movsx  eax, byte ptr [eax+5];
	__asm        test   eax, eax;
	__asm        je     _T3b;
// LINE 2238:
	__asm        jmp    _T25;
_T25:
	this->PlaneClass::UnlinkFromCell((this + 0x20));
// LINE 2239:
	this->flags[1] = 0x0;
// LINE 2241:
_T3b:
	return;
}

// FUNCTION: COPTER_D 0x0052d456
int32_t PlaneClass::InitializeInstance(long mapx, long mapy, int32_t instanceID) {
	/*bp-0x24*/  /*packed*/ struct VRObjInfo objectInfo; // 0x24 bytes
	/*bp-0x28*/  int32_t object;

// LINE 2272:
	this->dyObj.mesh = 0x0;
// LINE 2294:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x54];
	__asm        push   eax;
	__asm        call   _VRGetObject;
	__asm        add    esp, 4;
	__asm        mov    object, eax;
// LINE 2296:
	__asm        cmp    object, 0;
	__asm        jne    _T5a;
// LINE 2300:
	_assert(0x5b7804, 0x5b77e0, 0x8fc);
	__asm        jmp    _T53;

	__asm        jmp    _T53;
// LINE 2302:
_T53:
	return 0x0;
// LINE 2305:
_T5a:
	this->dyObj.mesh = object;
// LINE 2363:
	__asm        mov    eax, instanceID;
	__asm        mov    ecx, this;
	__asm        mov    [ecx+0x66], ax;
// LINE 2364:
	this->dyObj.flags = 0x101;
// LINE 2365:
	_VRIdentity4x4((this + 0x7c));
// LINE 2367:
	_VRGetObjInfo(object, objectInfo.Faces);
// LINE 2368:
	this->dyObj.radius = objectInfo.Radius;
// LINE 2389:
	this->beamTimer = 0x3c0000;
// LINE 2390:
	this->currentLocation.x = mapx;
// LINE 2391:
	this->currentLocation.y = mapy;
// LINE 2392:
	this->currentCell.x = mapx;
// LINE 2393:
	this->currentCell.y = mapy;
// LINE 2394:
	this->desiredSpeed = 0x780000;
// LINE 2395:
	this->shieldCtr = 0x1;
// LINE 2396:
	this->missileHits = 0x0;
// LINE 2397:
	this->smokeTime = 0xb40000;
// LINE 2401:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x54];
	__asm        mov    [ebp-0x30], eax;
	__asm        jmp    _T125;
// LINE 2404:
_T102:
	this->beamDelay = 0x3c0000;
// LINE 2405:
	__asm        jmp    _T144;
// LINE 2408:
_T111:
	this->beamDelay = 0x3c0000;
// LINE 2409:
	__asm        jmp    _T144;
// LINE 2410:
	__asm        jmp    _T144;
_T125:
	__asm        cmp    dword ptr [ebp-0x30], 0x12E;
	__asm        je     _T102;

	__asm        cmp    dword ptr [ebp-0x30], 0x17C;
	__asm        je     _T111;

	__asm        jmp    _T144;
// LINE 2415:
_T144:
	__asm        cmp    mapx, 0xFFFFFFFF;
	__asm        je     _T196;

	__asm        cmp    mapy, 0xFFFFFFFF;
	__asm        je     _T196;
// LINE 2419:
	this->PlaneClass::LinkToCell((this + 0x20));
// LINE 2423:
	this->PlaneClass::AdjustCurrentPosition();
// LINE 2424:
	this->PlaneClass::AdjustNextPosition();
// LINE 2425:
	this->PlaneClass::AdjustNextAltitude();
// LINE 2429:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax];
	__asm        mov    ecx, this;
	__asm        call   dword ptr [eax+8];
// LINE 2433:
	this->flags[1] = 0x1;
// LINE 2435:
	__asm        jmp    _T19d;
// LINE 2439:
_T196:
	this->flags[1] = 0x0;
// LINE 2448:
_T19d:
	__asm        mov    eax, this;
	__asm        mov    ecx, instanceID;
	__asm        mov    planes[0][ecx*4], eax;
// LINE 2451:
	this->flags[0] = 0x1;
// LINE 2453:
	return 0x1;
// LINE 2454:
}

// FUNCTION: COPTER_D 0x0052d618
void PlaneClass::HandleUFOStuff() {
	/*bp-0xc*/   /*packed*/ struct Point3d vec; // 0xc bytes

// LINE 2477:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x4C], 0;
	__asm        je     _T34;
// LINE 2479:
	this->shieldCtr--;
// LINE 2480:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x4C], 0;
	__asm        jne    _T34;
// LINE 2482:
	this->PlaneClass::TurnOffShields();
// LINE 2487:
_T34:
	__asm        xor    eax, eax;
	__asm        sub    eax, LoopTime;
	__asm        neg    eax;
	__asm        mov    ecx, this;
	__asm        sub    [ecx+0x48], eax;
// LINE 2488:
	__asm        mov    eax, this;
	__asm        cmp    dword ptr [eax+0x48], 0;
	__asm        jge    _T9b;
// LINE 2491:
	vec.z = 0x0;
	vec.x = vec.z;
// LINE 2492:
	vec.y = 0xffff0000;
// LINE 2494:
	this->smokeTime = 0xb40000;
// LINE 2504:
	S3MissileStart(0xb, (this + 0x20), (this + 0x70), vec.x, 0x1, (this + 0x58), 0x12c0000, -0x1);
// LINE 2506:
_T9b:
	return;
}

// FUNCTION: COPTER_D 0x0052d6bd
void CreatePlaneInstance(int32_t instanceID) {
// LINE 2554:
	PlaneClass::CreateInstance(instanceID);
// LINE 2555:
	return;
}

// FUNCTION: COPTER_D 0x0052d6d9
int32_t S3PlaneCrashWhenReady(long mission_id) {
// LINE 2574:
	return PlaneClass::FindPlaneToCrash(mission_id);
// LINE 2575:
}

// FUNCTION: COPTER_D 0x0052d6f5
void ResetAllPlanes() {
// LINE 2597:
	PlaneClass::ResetAll();
// LINE 2598:
	return;
}

// FUNCTION: COPTER_D 0x0052d70a
void ItterateAllPlanes() {
// LINE 2620:
	PlaneClass::ItterateAll();
// LINE 2621:
	return;
}

// FUNCTION: COPTER_D 0x0052d71f
void PlaneClass::HitDispatch(long hitter_type, /*packed*/ struct _DYOBJ_INST *dyhitter, /*packed*/ struct _DYOBJ_INST *dyhittee, long mission_id, long xtra_msg) {
// LINE 2635:
	__asm        mov    eax, hitter_type;
	__asm        mov    [ebp-0x18], eax;
	__asm        jmp    _T1f8;
// LINE 2638:
_T14:
	__asm        jmp    _T24c;
// LINE 2640:
_T19:
	__asm        jmp    _T24c;
// LINE 2642:
_T1e:
	__asm        jmp    _T24c;
// LINE 2644:
_T23:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T48;

	__asm        jmp    _Td4;

	__asm        jmp    _T57;
_T48:
	__asm        jmp    _T57;

	__asm        cmp    dword ptr [ebp-4], 0;
	__asm        je     _Td4;
// LINE 2646:
_T57:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    dword ptr [eax+0x4C], 3;
// LINE 2647:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    ecx, planes[0][eax*4];
	__asm        call   PlaneClass::TurnOnShields;
// LINE 2648:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        inc    dword ptr [eax+0x50];
// LINE 2649:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        cmp    dword ptr [eax+0x50], 0xA;
	__asm        jl     _Tcf;
// LINE 2650:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    [ebp-0xC], eax;
	__asm        mov    eax, [ebp-0xC];
	__asm        mov    byte ptr [eax+6], 1;
	__asm        mov    eax, [ebp-0xC];
	__asm        mov    dword ptr [eax+0x3C], 0xFFFFFFFF;
	__asm        jmp    _Tcf;
// LINE 2652:
_Tcf:
	__asm        jmp    _T110;
// LINE 2655:
_Td4:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    dword ptr [eax+0x50], 1;
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    [ebp-0x10], eax;
// LINE 2656:
	__asm        mov    eax, [ebp-0x10];
	__asm        mov    byte ptr [eax+6], 1;
	__asm        mov    eax, [ebp-0x10];
	__asm        mov    dword ptr [eax+0x3C], 0xFFFFFFFF;
	__asm        jmp    _T110;
// LINE 2658:
_T110:
	__asm        jmp    _T24c;
// LINE 2660:
_T115:
	__asm        jmp    _T24c;
// LINE 2662:
_T11a:
	__asm        jmp    _T24c;
// LINE 2664:
_T11f:
	__asm        jmp    _T24c;
// LINE 2666:
_T124:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        cmp    dword ptr [eax+0x54], 0x12E;
	__asm        jne    _T149;

	__asm        jmp    _T185;

	__asm        jmp    _T158;
_T149:
	__asm        jmp    _T158;

	__asm        cmp    dword ptr [ebp-8], 0;
	__asm        je     _T185;
// LINE 2668:
_T158:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    dword ptr [eax+0x4C], 3;
// LINE 2669:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    ecx, planes[0][eax*4];
	__asm        call   PlaneClass::TurnOnShields;
// LINE 2671:
	__asm        jmp    _T1c1;
// LINE 2674:
_T185:
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    dword ptr [eax+0x50], 1;
	__asm        mov    eax, dyhittee;
	__asm        movsx  eax, word ptr [eax+0xE];
	__asm        mov    eax, planes[0][eax*4];
	__asm        mov    [ebp-0x14], eax;
// LINE 2675:
	__asm        mov    eax, [ebp-0x14];
	__asm        mov    byte ptr [eax+6], 1;
	__asm        mov    eax, [ebp-0x14];
	__asm        mov    dword ptr [eax+0x3C], 0xFFFFFFFF;
	__asm        jmp    _T1c1;
// LINE 2677:
_T1c1:
	__asm        jmp    _T24c;
// LINE 2679:
_T1c6:
	__asm        jmp    _T24c;
// LINE 2681:
_T1cb:
	__asm        jmp    _T24c;
// LINE 2683:
_T1d0:
	__asm        jmp    _T24c;
// LINE 2685:
_T1d5:
	__asm        jmp    _T24c;
// LINE 2687:
_T1da:
	__asm        jmp    _T24c;
// LINE 2689:
_T1df:
	__asm        jmp    _T24c;
// LINE 2691:
_T1e4:
	__asm        jmp    _T24c;
// LINE 2693:
_T1e9:
	__asm        jmp    _T24c;
// LINE 2695:
	__asm        jmp    _T24c;
// LINE 2696:
	__asm        jmp    _T24c;
_T1f8:
	__asm        cmp    dword ptr [ebp-0x18], 0xF;
	__asm        ja     _T24c;

	__asm        mov    eax, [ebp-0x18];
	__asm        jmp    _Switch_20c[0][eax*4];
// Switch pointers:
//   _T14
//   _T19
//   _T1e
//   _T23
//   _T115
//   _T11a
//   _T11f
//   _T124
//   _T1c6
//   _T1cb
//   _T1d0
//   _T1d5
//   _T1da
//   _T1df
//   _T1e4
//   _T1e9
// LINE 2698:
_T24c:
	return;
}

// FUNCTION: COPTER_D 0x0052d975
void PlaneHitDispatch(long hitter_type, /*packed*/ struct _DYOBJ_INST *dyhitter, /*packed*/ struct _DYOBJ_INST *dyhittee, long mission_id, long xtra_msg) {
// LINE 2708:
	PlaneClass::HitDispatch(hitter_type, dyhitter, dyhittee, mission_id, xtra_msg);
// LINE 2709:
	return;
}

// FUNCTION: COPTER_D 0x0052d9a1
void PlaneClass::TurnOnShields() {
	/*bp-0x24*/  /*packed*/ struct VRObjInfo oinfo; // 0x24 bytes
	/*bp-0x28*/  int32_t face;
	/*bp-0x2c*/  int32_t count;
	/*bp-0x4c*/  /*packed*/ struct VRFaceInfo finfo; // 0x20 bytes

// LINE 2728:
	_VRGetObjInfo(this->dyObj.mesh, oinfo.Faces);
// LINE 2729:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x60];
	__asm        push   eax;
	__asm        call   _VRGetFirstFace;
	__asm        add    esp, 4;
	__asm        mov    face, eax;
// LINE 2730:
_FOR_3d:
	for (count = 0x0; (oinfo.Faces > count); count++) {
		// LINE 2732:
			_VRGetFaceInfo(face, finfo.Face);
		// LINE 2733:
			__asm        cmp    finfo.Plotter, 0xB;
			__asm        jne    _T7d;
		// LINE 2735:
			__asm        and    finfo.Attribute, 0x7FFFFFFF;
		// LINE 2736:
			_VRSetFaceInfo(face, finfo.Face);
		// LINE 2738:
		_T7d:
			__asm        mov    eax, face;
			__asm        push   eax;
			__asm        call   _VRGetNextFace;
			__asm        add    esp, 4;
			__asm        mov    face, eax;
	}
// LINE 2740:
_T91:
	return;
}

// FUNCTION: COPTER_D 0x0052da3c
void PlaneClass::TurnOffShields() {
	/*bp-0x24*/  /*packed*/ struct VRObjInfo oinfo; // 0x24 bytes
	/*bp-0x28*/  int32_t face;
	/*bp-0x2c*/  int32_t count;
	/*bp-0x4c*/  /*packed*/ struct VRFaceInfo finfo; // 0x20 bytes

// LINE 2750:
	_VRGetObjInfo(this->dyObj.mesh, oinfo.Faces);
// LINE 2751:
	__asm        mov    eax, this;
	__asm        mov    eax, [eax+0x60];
	__asm        push   eax;
	__asm        call   _VRGetFirstFace;
	__asm        add    esp, 4;
	__asm        mov    face, eax;
// LINE 2752:
_FOR_3d:
	for (count = 0x0; (oinfo.Faces > count); count++) {
		// LINE 2754:
			_VRGetFaceInfo(face, finfo.Face);
		// LINE 2755:
			__asm        cmp    finfo.Plotter, 0xB;
			__asm        jne    _T81;
		// LINE 2757:
			finfo.Attribute = (finfo.Attribute | 0x80000000);
		// LINE 2758:
			_VRSetFaceInfo(face, finfo.Face);
		// LINE 2760:
		_T81:
			__asm        mov    eax, face;
			__asm        push   eax;
			__asm        call   _VRGetNextFace;
			__asm        add    esp, 4;
			__asm        mov    face, eax;
	}
// LINE 2762:
_T95:
	return;
}

// FUNCTION: COPTER_D 0x0052dadb
int32_t S3PlaneMIFFLoad(void * __ptr32 miffReader) {
// LINE 2773:
	return PlaneClass::MIFFLoad(miffReader);
// LINE 2774:
}

// FUNCTION: COPTER_D 0x0052daf7
int32_t S3PlaneMIFFSave(void * __ptr32 miffWriter) {
// LINE 2786:
	return PlaneClass::MIFFSave(miffWriter);
// LINE 2787:
}

// FUNCTION: COPTER_D 0x0052db13
int32_t PlaneClass::MIFFLoad(void * __ptr32 miffReader) {
	/*bp-0x4*/   /*packed*/ class PlaneClass *p;
	/*bp-0x8*/   int32_t ret;
	/*bp-0xc*/   int32_t i;

// LINE 2805:
	ret = ReadFirstMIFFChunk(miffReader, 0x504c414e, 0x62b6e8, 0xbc);
// LINE 2806:
	__asm        cmp    ret, 0;
	__asm        jne    _FOR_44;
// LINE 2807:
	return 0x0;
// LINE 2809:
_FOR_44:
	for (i = 0x0; (i < 0x2); i++) {
		// LINE 2812:
			__asm        mov    eax, i;
			__asm        mov    eax, planes[0][eax*4];
			__asm        mov    p, eax;
		// LINE 2813:
			__asm        mov    eax, 0x62B6E8;
			__asm        add    eax, 8;
			__asm        mov    ecx, p;
			__asm        add    ecx, 8;
			__asm        mov    edx, [eax];
			__asm        mov    [ecx], edx;
			__asm        mov    edx, [eax+4];
			__asm        mov    [ecx+4], edx;
			__asm        mov    eax, [eax+8];
			__asm        mov    [ecx+8], eax;
		// LINE 2814:
			p->remainingDist = PlaneClass::lsPlane.remainingDist;
		// LINE 2815:
			p->speed = PlaneClass::lsPlane.speed;
		// LINE 2816:
			p->desiredSpeed = PlaneClass::lsPlane.desiredSpeed;
		// LINE 2817:
			__asm        mov    eax, 0x62B6E8;
			__asm        add    eax, 0x20;
			__asm        mov    ecx, [eax];
			__asm        mov    eax, [eax+4];
			__asm        mov    edx, p;
			__asm        add    edx, 0x20;
			__asm        mov    [edx], ecx;
			__asm        mov    [edx+4], eax;
		// LINE 2818:
			__asm        mov    eax, 0x62B6E8;
			__asm        add    eax, 0x28;
			__asm        mov    ecx, [eax];
			__asm        mov    eax, [eax+4];
			__asm        mov    edx, p;
			__asm        add    edx, 0x28;
			__asm        mov    [edx], ecx;
			__asm        mov    [edx+4], eax;
		// LINE 2819:
			p->beamDelay = PlaneClass::lsPlane.beamDelay;
		// LINE 2820:
			p->beamTimer = PlaneClass::lsPlane.beamTimer;
		// LINE 2821:
			p->missionId = PlaneClass::lsPlane.missionId;
		// LINE 2822:
			p->altAdjustment = PlaneClass::lsPlane.altAdjustment;
		// LINE 2823:
			__asm        mov    eax, 0x62B6E8;
			__asm        add    eax, 0x40;
			__asm        mov    ecx, [eax];
			__asm        mov    eax, [eax+4];
			__asm        mov    edx, p;
			__asm        add    edx, 0x40;
			__asm        mov    [edx], ecx;
			__asm        mov    [edx+4], eax;
		// LINE 2824:
			p->smokeTime = PlaneClass::lsPlane.smokeTime;
		// LINE 2825:
			p->shieldCtr = PlaneClass::lsPlane.shieldCtr;
		// LINE 2826:
			p->missileHits = PlaneClass::lsPlane.missileHits;
		// LINE 2827:
			__asm        mov    eax, 0x62B6E8;
			__asm        add    eax, 0x70;
			__asm        mov    ecx, p;
			__asm        add    ecx, 0x70;
			__asm        mov    edx, [eax];
			__asm        mov    [ecx], edx;
			__asm        mov    edx, [eax+4];
			__asm        mov    [ecx+4], edx;
			__asm        mov    eax, [eax+8];
			__asm        mov    [ecx+8], eax;
		// LINE 2828:
			__asm        mov    edi, p;
			__asm        add    edi, 0x7C;
			__asm        lea    esi, PlaneClass::lsPlane.dyObj.matrix[0][0];
			__asm        mov    ecx, 0x10;
			__asm        rep movsd;
		// LINE 2829:
			reinterpret_cast<uint32_t>(p->flags[0]) = reinterpret_cast<uint32_t>(PlaneClass::lsPlane.flags[0]);
		// LINE 2831:
			__asm        jmp    _T174;
		_T174:
			__asm        mov    eax, p;
			__asm        movsx  eax, byte ptr [eax+5];
			__asm        test   eax, eax;
			__asm        je     _T192;
		// LINE 2833:
			p->PlaneClass::LinkToCell((p + 0x20));
		// LINE 2840:
		_T192:
			ret = ReadNextMIFFChunk(miffReader, 0x504c414e, 0x62b6e8, 0xbc);
		// LINE 2842:
			__asm        cmp    ret, 0;
			__asm        jne    _T1cb;

			__asm        cmp    i, 1;
			__asm        je     _T1cb;
		// LINE 2843:
			return 0x0;
		// LINE 2844:
		_T1cb:
	}
// LINE 2845:
_T1d0:
	return 0x1;
// LINE 2846:
}

// FUNCTION: COPTER_D 0x0052dcf2
int32_t PlaneClass::MIFFSave(void * __ptr32 miffWriter) {
	/*bp-0x4*/   int32_t ret;
	/*bp-0x8*/   int32_t i;

// LINE 2859:
_FOR_15:
	for (i = 0x0; (i < 0x2); i++) {
		// LINE 2864:
			__asm        push   0xBC;
			__asm        mov    eax, i;
			__asm        mov    eax, planes[0][eax*4];
			__asm        push   eax;
			__asm        push   0x504C414E;
			__asm        mov    eax, miffWriter;
			__asm        push   eax;
			__asm        call   WriteMIFFChunk;
			__asm        add    esp, 0x10;
			__asm        mov    ret, eax;
		// LINE 2865:
			__asm        cmp    ret, 0;
			__asm        jne    _T57;
		// LINE 2866:
			return 0x0;
		// LINE 2867:
		_T57:
	}
// LINE 2868:
_T5c:
	return 0x1;
// LINE 2869:
}



// Contribution: 1:0012cd60-0012cd98 Module: 163, 16 byte alignment, code, (comdat), execute, read, 
// FUNCTION: COPTER_D 0x0052dd60
// PlaneClass::`scalar deleting destructor'



// Contribution: 2:00004220-00004233 Module: 163, 8 byte alignment, initialized_data, (comdat), read, 
// vftable for PlaneClass @ 0x00593220
//   00: PlaneClass::`scalar deleting destructor' @ 0x0052dd60
//   01: PlaneClass::ItterateFSM @ 0x0052b9a3
//   02: PlaneClass::AdjustSpeed @ 0x0052baf5
//   03: PlaneClass::IsPathClear @ 0x0052bb2e
//   04: PlaneClass::BeamToWithinCameraRange @ 0x0052d08b
//   5 entries



// Contribution: 3:000000a8-000000ab Module: 163, 4 byte alignment, initialized_data, read, write, 
// GLOBAL: COPTER_D 0x005970a8
static void (*$S9)() = { 0 /* todo */ };



// Contribution: 3:00020690-0002080a Module: 163, 8 byte alignment, initialized_data, read, write, 
// GLOBAL: COPTER_D 0x005b7690
/*packed*/ struct Point2d PlaneClass::lastScannedLocation = { 0 /* todo */ };

// GLOBAL: COPTER_D 0x005b7698
int32_t PlaneClass::sPlaneModelSet = 0;



// Contribution: 3:0002080c-0002082c Module: 163, 4 byte alignment, initialized_data, (comdat), read, write, 
// string literal: "C:\Copter\Source\Game\S3MOBILE.H"


// Contribution: 3:000946e8-000947b0 Module: 163, 8 byte alignment, uninitialized_data, read, write, 
// GLOBAL: COPTER_D 0x0062b6e8
/*packed*/ class PlaneClass PlaneClass::lsPlane;

// GLOBAL: COPTER_D 0x0062b7a8
/*packed*/ class PlaneClass *planes[2];

// GLOBAL: COPTER_D 0x0062b7b0
// $S6
